...
Use this skill when a task involves **UI structure, visual design decisions, interaction patterns, or user-experience quality control**.
...
- Choosing a color scheme, type system, spacing scale, or layout system
...
- The UI looks "unprofessional" but the reason isn't clear
- You've received feedback about usability or experience
- You're polishing UI quality before a release
- You need to align design across platforms (Web / iOS / Android)
- You're building a design system or a reusable component library

### Skip

This skill is unnecessary when:
...
**Rule of thumb**: if the task changes **how a feature looks, how it feels to use, how it moves, or how it is interacted with**, use this skill.

## Rule Categories by Priority

*For human/AI reference: work through 1→10 to decide which category of rules to address first; query `--domain <Domain>` when you need the details. The scripts do not read this table.*
...
- The default icon library is **Phosphor (`@phosphor-icons/react`)**. The icons listed in `src/ui-ux-pro-max/data/icons.csv` are only the commonly recommended ones, not the complete set.
- When the recommendation table has no suitable icon:
  ...
  - If Phosphor has no good option either, **Heroicons (`@heroicons/react`)** may be used as a fallback — keep the style consistent (outline/filled, stroke width, corner radius).
