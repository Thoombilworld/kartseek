...
import {
  ...
  TaxiSurgeZone,
  TaxiSurgeEvent,
  TaxiCancellationRule,
  TaxiWaitingFeeRule,
  TaxiFareBreakdown,
  TaxiDriverEarning,
  TaxiVendorSettlement,
  TaxiCommissionRecord,
  TaxiPaymentRecord,
  TaxiSosCase,
  TaxiDispute,
  TaxiAuditLog,
 ...
 */
const svcHost = (name: string): string => process.env[`${name}_SERVICE_HOST`] ?? '127.0.0.1';
    ...
    ClientsModule.register([
      {
        ...
        options: {
          host: svcHost('MARKETPLACE'),
          port: +(process.env.MARKETPLACE_TCP_PORT ?? 4002),
        },
      },
      {
        name: 'CART_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('CART'), port: +(process.env.CART_TCP_PORT ?? 4003) },
      },
      ...
      // order service side.
      {
        ...
        transport: Transport.TCP,
        options: { host: svcHost('ORDER'), port: +(process.env.ORDER_TCP_PORT ?? 4004) },
      },
      {
        name: 'LOYALTY_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('LOYALTY'), port: +(process.env.LOYALTY_TCP_PORT ?? 4005) },
      },
      {
        name: 'FRANCHISE_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('FRANCHISE'), port: +(process.env.FRANCHISE_TCP_PORT ?? 4006) },
      },
      {
        name: 'DOCTOR_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('DOCTOR'), port: +(process.env.DOCTOR_TCP_PORT ?? 4007) },
      },
      {
        name: 'GROCERY_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('GROCERY'), port: +(process.env.GROCERY_TCP_PORT ?? 4008) },
      },
      {
        name: 'PHARMACY_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('PHARMACY'), port: +(process.env.PHARMACY_TCP_PORT ?? 4010) },
      },
      {
        name: 'LOCATION_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('LOCATION'), port: +(process.env.LOCATION_TCP_PORT ?? 4013) },
      },
      {
        name: 'WALLET_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('WALLET'), port: +(process.env.WALLET_TCP_PORT ?? 4014) },
      },
      {
        name: 'ADMIN_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('ADMIN'), port: +(process.env.ADMIN_TCP_PORT ?? 4017) },
      },
      {
        name: 'RESTAURANT_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('RESTAURANT'), port: +(process.env.RESTAURANT_TCP_PORT ?? 4018) },
      },
      ...
      // seller_* message patterns, so this token resolves to Marketplace's TCP port.
      {
        name: 'SELLER_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('SELLER'), port: +(process.env.MARKETPLACE_TCP_PORT ?? 4002) },
      },
      {
        name: 'COMMISSION_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('COMMISSION'), port: +(process.env.COMMISSION_TCP_PORT ?? 4020) },
      },
      {
        name: 'PAYOUT_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('PAYOUT'), port: +(process.env.PAYOUT_TCP_PORT ?? 4021) },
      },
      {
        name: 'REFUND_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('REFUND'), port: +(process.env.REFUND_TCP_PORT ?? 4022) },
      },
      {
        name: 'SEARCH_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('SEARCH'), port: +(process.env.SEARCH_TCP_PORT ?? 4023) },
      },
      {
        name: 'REPORT_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('REPORT'), port: +(process.env.REPORT_TCP_PORT ?? 4024) },
      },
      {
        name: 'HOTEL_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('HOTEL'), port: +(process.env.HOTEL_TCP_PORT ?? 4025) },
      },
      {
        name: 'PAYMENT_SERVICE',
        transport: Transport.TCP,
        options: { host: svcHost('PAYMENT'), port: +(process.env.PAYMENT_TCP_PORT ?? 4026) },
      },
      {
        name: 'TAXI_SERVICE',
        transport: Transport.TCP,
