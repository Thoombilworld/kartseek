...
export class DeliveryController {
  ...
  @ApiBody({
    schema: {
      properties: {
        orderId: { type: 'string' },
        serviceType: { type: 'string', enum: ['food', 'grocery', 'pharmacy', 'marketplace'] },
      },
    },
  })
  async assignPartner(@Body() dto: { orderId: string; serviceType: string }) {
    const partnerId = `DP-${Math.floor(Math.random() * 10000)
      .toString()
      .padStart(4, '0')}`;
    const assignment = {
      orderId: dto.orderId,
  ...
  async getDeliveryStatus(@Param('orderId') orderId: string) {
    ...
    return status
      ? { success: true, delivery: status }
      : { success: false, reason: 'No delivery found' };
  }
  ...
  async updateStatus(
    ...
    await this.kafka.publish(KAFKA_TOPICS.DELIVERY_STATUS_UPDATED, {
      orderId,
      status: body.status,
  ...
  async getPartnerActive(@Param('partnerId') partnerId: string) {
    ...
    const keys = await this.redis.scanKeys('delivery:assignment:*');
  ...
  async getPartnerHistory(
    ...
    allHistory.sort(
      (a, b) =>
        new Date(b.updatedAt || b.assignedAt).getTime() -
        new Date(a.updatedAt || a.assignedAt).getTime(),
    );
  ...
  async estimateFee(@Body() dto: { distanceKm: number; weight?: number; serviceType?: string }) {
    ...
    return {
      ...
      fee,
      currency: 'INR',
      serviceType: dto.serviceType ?? 'marketplace',
  ...
  @ApiBody({
    schema: {
      properties: { lat: { type: 'number' }, lng: { type: 'number' }, heading: { type: 'number' } },
    },
  })
