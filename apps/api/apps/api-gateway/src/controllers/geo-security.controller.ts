...
import {
  ...
  GeoSecurityRule,
  GeoWhitelistedIp,
} from '../entities/geo-security.entities';
...
export class GeoSecurityController {
  ...
  async checkGeo(@Req() req: any): Promise<GeoCheckResult> {
    ...
    const isSuspicious =
      vpnAnalysis.isVpn || vpnAnalysis.isProxy || vpnAnalysis.isTor || vpnAnalysis.isDatacenter;
  ...
  async verifyLocation(@Req() req: any, @Body() dto: LocationVerifyRequest) {
    ...
    if (isMismatch || countriesDiffer) {
      await this.logEvent({
        ...
        ipCountry: geoData.country,
        ipCity: geoData.city,
        gpsCountry: dto.gpsCountry ?? undefined,
        gpsCity: dto.gpsCity ?? undefined,
        gpsLat: dto.gpsLat,
        gpsLng: dto.gpsLng,
        isVpn: false,
        isProxy: false,
    ...
    return {
      ...
      recommendation: isMismatch
        ? 'Location mismatch detected. GPS location will be used for services.'
        : 'Location verified.',
    };
  }
  ...
  //
  // THESE SIX USED TO CARRY `@UseGuards(JwtAuthGuard)` AND NO `@Roles`.
  //
  // The class binds nothing, so authentication was the whole of the check: any
  // signed-in account of any role could read the geo security event log, read
  // and **rewrite** a geo security rule, and **whitelist an arbitrary IP** out
  // of the platform's own VPN/proxy/geo enforcement — a one-request bypass from
  // a customer token (whole-branch review, finding A-2). Task 7 hardened the
  // other security console (`ddos-admin.controller.ts`) and this one was never
  // in any task's file list; it was invisible to the market-scope gate for the
  // same reason the hotel routes were, and `route-exposure.regression.spec.ts`
  // allowlisted `/^\/geo\//` wholesale as "pre-login geo/VPN check", so even
  // deleting the `JwtAuthGuard` would not have failed a spec. That prefix is
  // now narrowed to the two routes that really are pre-login.
  //
  // The rule set matches `ddos-admin.controller.ts` exactly, because it is the
  // same kind of object: `SUPER_ADMIN` plus `security.manage` (the threat
  // board and the geo policy are both the map of the platform's defences, and
  // an ADMIN holding neither has no business in either), `refuseLockedAdmin`
  // ahead of every handler, and `@GlobalEntity` on the reads. `refuseLockedAdmin`
  // cannot fire under a `SUPER_ADMIN`-only gate — `marketScopeOf` never reports
  // a SUPER_ADMIN as locked — and is written anyway: it is the line that has to
  // already be there the day somebody widens the role gate to ADMIN.

  @Get('admin/events')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.SUPER_ADMIN, 'perm:security.manage')
  @GlobalEntity('geo security policy is platform-wide')
  @ApiBearerAuth('JWT')
  ...
  async getSecurityEvents(
    ...
    refuseLockedAdmin(req, 'the geo security event log');
    ...
    if (this.isDb()) {
      try {
        ...
        this.logger.warn(
          `Reading geo-security events from the database failed; serving the recent events kept in Redis instead: ${String(err)}`,
        );
      }
    }
  ...
  async getRules(@Req() req: any) {
    refuseLockedAdmin(req, 'the geo security rule set');
    if (this.isDb()) {
      try {
        return await this.em!.find(GeoSecurityRule, { where: { isActive: true } });
      } catch (err) {
        this.logger.warn(
          `Reading geo-security rules from the database failed; answering with the built-in default rules: ${String(err)}`,
        );
      }
    }
  ...
  async updateRule(
    ...
    // No `@GlobalEntity` on a write: the marker is read-only by the market-scope
    // regression spec's own rule, and `refuseLockedAdmin` is what a global write
    // owes a region-locked caller.
    refuseLockedAdmin(req, 'a geo security rule');
    const ruleKey = String(dto?.ruleKey ?? '').trim();
    if (!ruleKey) {
      throw new BadRequestException('ruleKey is required.');
    }
    // Named keys, never a spread of the body. `em.update(..., dto)` passed the
    // request object straight through, so any column on `GeoSecurityRule` —
    // `isActive` and `id` included — was writable from the wire on a security
    // rule. Only these four are a rule's own editable content.
    const patch = {
    ...
    if (this.isDb()) {
      try {
        const existing = await this.em!.findOne(GeoSecurityRule, { where: { ruleKey } });
        if (existing) {
          await this.em!.update(GeoSecurityRule, existing.id, patch);
        } else {
          await this.em!.save(
            GeoSecurityRule,
            this.em!.create(GeoSecurityRule, { ...patch, ruleKey, isActive: true }),
          );
        }
      } catch (err) {
  ...
  async getWhitelist(@Req() req: any) {
    refuseLockedAdmin(req, 'the geo IP whitelist');
    if (this.isDb()) {
      try {
        return await this.em!.find(GeoWhitelistedIp, { where: { isActive: true } });
      } catch (err) {
        this.logger.warn(
          `Reading the IP whitelist from the database failed; answering with the Redis whitelist instead: ${String(err)}`,
        );
      }
    }
  ...
  async addWhitelist(@Req() req: any, @Body() dto: { ip: string; reason?: string }) {
    refuseLockedAdmin(req, 'the geo IP whitelist');
    // An entry with no address whitelists nothing and reported success. The
    // shape is checked here because this body has no DTO class for the global
    // ValidationPipe to read.
    if (!String(dto?.ip ?? '').trim()) {
    ...
    if (this.isDb()) {
      try {
        await this.em!.save(
          GeoWhitelistedIp,
          this.em!.create(GeoWhitelistedIp, {
            ip: dto.ip,
            reason: dto.reason,
  ...
  async getStats(@Req() req: any) {
    refuseLockedAdmin(req, 'the geo security dashboard');
  ...
  private readonly defaultRules = [
    {
      ruleKey: 'vpn_policy',
      ruleValue: 'block',
      description: 'Action when VPN is detected: block | warn | allow',
    },
    ...
    {
      ruleKey: 'datacenter_policy',
      ruleValue: 'warn',
      description: 'Action for datacenter/hosting IPs',
    },
    {
      ruleKey: 'mismatch_threshold_km',
      ruleValue: '500',
      description: 'Max allowed GPS-to-IP distance in km',
    },
    {
      ruleKey: 'log_all_checks',
      ruleValue: 'false',
      description: 'Log all checks, not just suspicious ones',
    },
  ];
  ...
  private async isWhitelisted(ip: string): Promise<boolean> {
    ...
    if (this.isDb()) {
      try {
        ...
        this.logger.warn(
          `Whitelist lookup for ${ip} failed in the database; treating the IP as not whitelisted: ${String(err)}`,
        );
      }
    }
    return false;
  ...
  private async lookupIp(ip: string): Promise<any> {
    ...
    try {
      ...
      this.logger.warn(
        `IP lookup for ${ip} failed; caching an empty geolocation for it for one hour, so its country stays unknown: ${String(e)}`,
      );
    }
  ...
  private analyzeVpnIndicators(
    geoData: any,
    req: any,
  ): { isVpn: boolean; isProxy: boolean; isTor: boolean; isDatacenter: boolean } {
    let isVpn = false;
    let isProxy = false;
    let isTor = false;
  ...
  private calculateThreatLevel(analysis: {
    isVpn: boolean;
    isProxy: boolean;
    isTor: boolean;
    isDatacenter: boolean;
  ...
  private async getPolicy(countryCode: string | null): Promise<'block' | 'warn' | 'allow'> {
    ...
    if (this.isDb()) {
      try {
        ...
        this.logger.warn(
          `Reading the vpn_policy rule from the database failed; applying the default policy "block": ${String(err)}`,
        );
      }
    }
  ...
  private async getMismatchThreshold(): Promise<number> {
    ...
    if (this.isDb()) {
      try {
        ...
        this.logger.warn(
          `Reading the mismatch_threshold_km rule from the database failed; using the default 500 km: ${String(err)}`,
        );
      }
    }
    return 500;
  ...
  private async logEvent(data: Partial<GeoSecurityEvent>): Promise<void> {
    ...
    if (this.isDb()) {
      try {
        ...
        this.logger.error(
          `Persisting geo-security event ${data.eventType} for ${data.ip} failed: ${String(err)}`,
        );
      }
    }
