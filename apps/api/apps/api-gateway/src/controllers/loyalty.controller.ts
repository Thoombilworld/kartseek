import { Controller, Get, Post, Body, Inject, UseGuards, Req, Query, Param, Logger, HttpException, HttpStatus } from '@nestjs/common';
...
export class LoyaltyGatewayController {
  ...
  private async send<T>(cmd: string, payload: object): Promise<T> {
    try {
      return await lastValueFrom(
        this.loyaltyClient.send<T>({ cmd }, payload).pipe(
          timeout(5000),
          catchError(rpcCatch('Loyalty service unavailable')),
        ),
      );
    } catch (err) {
  ...
  async awardPoints(@Body() dto: { points: number; reason: string; orderId?: string }, @Req() req: any) {
    const userId = req.user.id;
  ...
  async redeemPoints(@Body() dto: { points: number }, @Req() req: any) {
    ...
    const userId = req.user.id;
    const result = await this.send<{ success: boolean; reason?: string }>(
      'redeem_loyalty_points', { userId, points },
    );
    if (result && result.success === false) {
      throw new HttpException(result.reason ?? 'Points could not be redeemed', HttpStatus.BAD_REQUEST);
    }
    return result;
  }
