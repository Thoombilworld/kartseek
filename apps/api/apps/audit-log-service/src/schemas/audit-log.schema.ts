...
AuditLogSchema.pre('deleteOne', function () {
  throw new Error('AuditLog cannot be deleted');
});
AuditLogSchema.index({ actorId: 1, createdAt: -1 });
AuditLogSchema.index({ entityType: 1, entityId: 1 });
AuditLogSchema.index({ actionType: 1 });
/**
 * The shape every scoped read takes: `{ country: { $in: [market, 'ALL'] } }`
 * sorted by `createdAt: -1`. Without it each page of the admin audit console is
 * a collection scan plus an in-memory sort, which does not merely get slow — it
 * starts failing outright once the sorted set passes Mongo's 32MB limit, and an
 * audit trail is append-only, so that day arrives on its own.
 */
AuditLogSchema.index({ country: 1, createdAt: -1 });
