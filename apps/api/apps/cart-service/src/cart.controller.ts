...
export class CartController {
  ...
  @Get('user/:userId') getCart(@Param('userId') userId: string) {
  ...
  @Delete('user/:userId/items/:productId') removeItem(
    @Param('userId') userId: string,
    @Param('productId') pid: string,
    @Query('variantId') vid?: string,
  ) {
    return this.svc.removeItem(userId, pid, vid);
  }
  @Delete('user/:userId') clearCart(@Param('userId') userId: string) {
    return this.svc.clearCart(userId);
  }

  @MessagePattern({ cmd: 'add_to_cart' }) msgAdd(@Payload() d: EmptyMessage) {
    return this.svc.addItem(d.userId, d);
  }
  @MessagePattern({ cmd: 'get_cart' }) msgGet(@Payload() d: { userId: string }) {
    return this.svc.getCart(d.userId);
  }
  @MessagePattern({ cmd: 'clear_cart' }) msgClear(@Payload() d: { userId: string }) {
    return this.svc.clearCart(d.userId);
  }
  ...
  msgUpdate(
    ...
    return this.svc.updateItemQuantity(
      d.userId,
      d.itemId,
      Number(d.quantity),
      d.variantId,
  ...
  msgRemove(
    @Payload() d: { userId: string; itemId: string; variantId?: string; regionCode?: string },
  ) {
    if (!d?.userId || !d?.itemId) throw new BadRequestException('userId and itemId are required');
    return this.svc.removeItem(d.userId, d.itemId, d.variantId, d.regionCode);
  }
}
