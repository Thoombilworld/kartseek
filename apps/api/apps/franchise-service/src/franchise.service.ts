...
export class FranchiseService {
  ...
  /** Presentation helper — the module services return raw numbers. */
  private toLakhs(amount: number): string {
    return `₹${(Number(amount || 0) / 100000).toFixed(1)}L`;
  }
  ...
  async getMarketplaceKpis(franchiseId: string) {
    ...
    return {
      ...
      retailRevenue: this.toLakhs(kpis.retailRevenue),
  ...
  async getGroceryKpis(franchiseId: string) {
    ...
    return {
      ...
      totalProducts: kpis.totalProducts,
      totalOrders: kpis.totalOrders,
      revenue: this.toLakhs(kpis.revenue),
    };
  }
