...
async function seed() {
  ...
  const rolesData = [
    ...
    { name: 'DELIVERY_PARTNER', permissions: ['accept_task', 'update_status', 'view_earnings', 'collect_cod', 'sos'] },
  ];
  ...
  const partnersData = [
    { name: 'Rahul Sharma',       phone: '+91981234001', email: 'rahul.sharma@email.com',    status: 'APPROVED', regionCode: 'IN-MH' },
    { name: 'Priya Patel',    phone: '+91981234002', email: 'priya.p@email.com',      status: 'APPROVED', regionCode: 'IN-MH' },
    { name: 'Obi Nnamdi',        phone: '+234801234003', email: 'obi.nnamdi@email.com',     status: 'APPROVED', regionCode: 'NG-LA' },
    { name: 'Fatima Khan',       phone: '+91981234004', email: 'fatima.khan@email.com',    status: 'APPROVED', regionCode: 'IN-MH' },
    { name: 'Suresh Kumar',    phone: '+91981234005', email: 'suresh.k@email.com',       status: 'APPROVED', regionCode: 'IN-MH' },
    { name: 'Ada Eze',           phone: '+234801234006', email: 'ada.eze@email.com',        status: 'APPROVED', regionCode: 'NG-LA' },
    { name: 'Hamisi Juma',       phone: '+255701234007', email: 'hamisi.j@email.com',       status: 'APPROVED', regionCode: 'TZ-DA' },
    { name: 'Anjali Gupta',      phone: '+91981234008', email: 'anjali.g@email.com',        status: 'PENDING',  regionCode: 'IN-MH' },
    { name: 'Chinedu Okwu',      phone: '+234801234009', email: 'chinedu.o@email.com',      status: 'BLOCKED',  regionCode: 'NG-LA' },
    { name: 'Amina Sheikh',      phone: '+91981234010', email: 'amina.s@email.com',        status: 'APPROVED', regionCode: 'IN-MH' },
  ];
  ...
  for (let i = 0; i < savedPartners.length; i++) {
    ...
    const roles = i < 5 ? ['TAXI_DRIVER'] : i < 8 ? ['DELIVERY_PARTNER'] : ['TAXI_DRIVER', 'DELIVERY_PARTNER'];
    const userId = `auth-user-${6000 + i}`;

    let pu = await userRepo.findOneBy({ partnerId: partner.id, userId });
    if (!pu) {
      pu = await userRepo.save(userRepo.create({
    ...
    for (const roleName of roles) {
      ...
      if (role) {
        const existingAssignment = await roleAssignRepo.findOneBy({ partnerUserId: pu.id, roleId: role.id });
        if (!existingAssignment) {
          await roleAssignRepo.save(roleAssignRepo.create({ partnerUserId: pu.id, roleId: role.id }));
        }
      }
    }
  }
  ...
  const baseLocations = [
    ...
    { lat: -6.8160, lng: 39.2803 },  // Dar es Salaam
  ];
  let locCount = 0;
  for (const partner of savedPartners) {
    if (partner.status !== 'APPROVED') continue;
