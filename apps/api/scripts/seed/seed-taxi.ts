...
async function seed() {
  ...
  const vendorData = [
    {
      name: 'FastRide Nigeria',
      status: 'APPROVED',
      franchiseId: null as string | null,
      regionCode: 'NG-LA',
    },
    {
      name: 'SafeCab Nigeria',
      status: 'APPROVED',
      franchiseId: null as string | null,
      regionCode: 'NG-AB',
    },
    {
      name: 'Premium Rides NG',
      status: 'APPROVED',
      franchiseId: null as string | null,
      regionCode: 'NG-LA',
    },
  ...
  for (const vu of vendorUsers) {
    ...
    if (!existing) {
      await vendorUserRepo.save(vendorUserRepo.create({ ...vu, userId: '' }));
      vuCount++;
    }
  }
  ...
  const vehicleData = [
    ...
    },
    {
      ...
      model: 'Toyota Axio 2020',
      type: 'comfort',
      vendorIdx: 3,
    },
    {
      driverIdx: 7,
      plateNumber: 'MSA-TT-001',
      model: 'Piaggio Ape',
      type: 'economy',
      vendorIdx: 4,
    },
  ...
  for (const fr of fareData) {
    ...
    if (!existing) {
      await fareRuleRepo.save(fareRuleRepo.create({ ...fr, zoneId: 'default' }));
      frCount++;
    }
  }
  ...
  const surgeRules = [
    {
      zoneId: savedZones[0]?.id,
      name: 'Morning Rush Lagos',
      multiplier: 1.3,
      isActive: true,
      capLimit: 3.0,
    },
  ...
  for (const sr of surgeRules) {
    ...
    if (!existing) {
      await surgeRuleRepo.save(surgeRuleRepo.create(sr));
      srCount++;
    }
  }
  ...
  for (const cr of cancelRules) {
    ...
    if (!existing) {
      await cancelRuleRepo.save(cancelRuleRepo.create(cr));
      crCount++;
    }
  }
  ...
  for (const wr of waitingRules) {
    ...
    if (!existing) {
      await waitingRuleRepo.save(waitingRuleRepo.create(wr));
      wrCount++;
    }
  }
  ...
  const locations = [
    ...
    { lat: 6.44, lng: 3.42, name: 'Lekki Phase 1, Lagos' },
    { lat: -1.2921, lng: 36.8219, name: 'Mumbai Central' },
    { lat: -1.2634, lng: 36.8073, name: 'Andheri West, Mumbai' },
    { lat: 19.076, lng: 72.8777, name: 'Mumbai Central' },
    { lat: 28.6139, lng: 77.209, name: 'Connaught Place, Delhi' },
  ];
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const drop =
      locations[
        (locations.indexOf(pickup) + 1 + Math.floor(Math.random() * (locations.length - 1))) %
          locations.length
      ];
    ...
    const finalFare =
      status === 'RIDE_COMPLETED' ? +(fareEstimate + Math.random() * 500).toFixed(2) : 0;
