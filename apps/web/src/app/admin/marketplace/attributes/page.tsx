...
import {
  Layers, Plus, Edit2, Trash2, Search, ChevronDown, ChevronRight, Save, X,
  ToggleRight, ToggleLeft, Filter, Type, Hash, List, Calendar, Palette, Ruler,
  ToggleLeft as TL, Boxes,
} from 'lucide-react';
...
type AttrType = 'TEXT' | 'NUMBER' | 'SELECT' | 'MULTI_SELECT' | 'BOOLEAN' | 'COLOR' | 'DATE' | 'RANGE';
...
const typeIcons: Record<AttrType, React.ReactNode> = {
  TEXT: <Type className="w-3.5 h-3.5" />, NUMBER: <Hash className="w-3.5 h-3.5" />,
  SELECT: <List className="w-3.5 h-3.5" />, MULTI_SELECT: <List className="w-3.5 h-3.5" />,
  BOOLEAN: <TL className="w-3.5 h-3.5" />, COLOR: <Palette className="w-3.5 h-3.5" />,
  DATE: <Calendar className="w-3.5 h-3.5" />, RANGE: <Ruler className="w-3.5 h-3.5" />,
};

const typeLabels: Record<AttrType, string> = {
  TEXT: 'Text', NUMBER: 'Number', SELECT: 'Single Select', MULTI_SELECT: 'Multi Select',
  BOOLEAN: 'Yes/No', COLOR: 'Colour', DATE: 'Date', RANGE: 'Range',
};
...
function AttrModal({ attr, categories, defaultCategoryId, onSave, onClose, saving }: {
  attr?: Attribute;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-slate-200">
          <h3 className="text-lg font-black text-slate-900">{attr ? 'Edit Attribute' : 'Add Attribute'}</h3>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5" htmlFor="attr-name">Name *</label>
              <input id="attr-name" value={name} onChange={(e) => { setName(e.target.value); if (!attr) setSlug(slugify(e.target.value)); }}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. RAM" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5" htmlFor="attr-slug">Slug</label>
              <input id="attr-slug" value={slug} onChange={(e) => setSlug(slugify(e.target.value))}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none font-mono" placeholder="e.g. ram" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5" htmlFor="attr-type">Type</label>
              <select id="attr-type" value={type} onChange={(e) => setType(e.target.value as AttrType)}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none">
                {(Object.keys(typeLabels) as AttrType[]).map((t) => <option key={t} value={t}>{typeLabels[t]}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5" htmlFor="attr-category">Category</label>
              ...
              <select id="attr-category" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none">
          ...
          {hasValues && (
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5">Values ({options.length})</label>
              <div className="flex flex-wrap gap-1.5 mb-2">
                {options.map((o) => (
                  <span key={o.value} className="inline-flex items-center gap-1.5 bg-slate-100 text-slate-700 text-xs font-medium px-2.5 py-1 rounded-lg">
                    {o.hex && <span className="w-3 h-3 rounded-full border border-slate-300" style={{ backgroundColor: o.hex }} />}
                    {o.label}
                    <button onClick={() => setOptions((prev) => prev.filter((p) => p.value !== o.value))}
                      className="text-slate-400 hover:text-red-500" aria-label={`Remove ${o.label}`}><X className="w-3 h-3" /></button>
                  </span>
                ))}
              </div>
              <div className="flex gap-2">
                <input value={newVal} onChange={(e) => setNewVal(e.target.value)}
                  ...
                  className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none" placeholder="Add a value…" />
                ...
                {isColour && (
                  <input type="color" value={newHex} onChange={(e) => setNewHex(e.target.value)}
                    aria-label="Swatch colour" title="Swatch colour"
                    className="w-12 h-11 border border-slate-200 rounded-xl cursor-pointer" />
                )}
                <button onClick={addValue} className="bg-blue-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold" aria-label="Add value"><Plus className="w-4 h-4" /></button>
              </div>
            </div>
          )}
          ...
          {(type === 'NUMBER' || type === 'RANGE') && (
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5" htmlFor="attr-unit">Unit</label>
              <input id="attr-unit" value={unit} onChange={(e) => setUnit(e.target.value)}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none" placeholder="e.g. mAh, inches, kg" />
            </div>
          )}
          ...
          <label className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
            ...
            <input type="checkbox" checked={isVariantAxis && hasValues} disabled={!hasValues}
              onChange={(e) => setVariantAxis(e.target.checked)} className="mt-0.5 rounded" />
            <span>
              <span className="text-sm font-bold text-slate-800 flex items-center gap-1.5"><Boxes className="w-4 h-4 text-blue-600" /> Variant axis</span>
...
export default function AttributesPage() {
  ...
  const categories: AdminCategoryRow[] = useMemo(
    () => rowsOf(categoriesRes.data).map((c: any) => ({
      id: String(c.id), name: c.name ?? c.slug ?? 'Category', slug: c.slug, icon: c.icon,
    })),
    [categoriesRes.data],
  );
  ...
  const groups = useMemo(() => {
    ...
    return [...byCategory.entries()].map(([key, list]) => ({
      key,
      categoryId: key === '__global__' ? null : key,
      name: key === '__global__'
        ? 'All categories (global)'
        : (categories.find((c) => c.id === key)?.name ?? list[0]?.category?.name ?? 'Unassigned category'),
  ...
  const filteredGroups = useMemo(() => {
    ...
    if (!q) return groups;
    return groups
      .map((g) => ({ ...g, attributes: g.attributes.filter((a) => a.name.toLowerCase().includes(q) || a.slug.includes(q)) }))
  ...
  const reload = useCallback(() => { attributesRes.refetch(); }, [attributesRes]);
  ...
  const save = async (data: Partial<Attribute>) => {
    ...
    try {
      ...
      if (ok !== null) { setShowModal(false); setEditAttr(undefined); reload(); }
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (attr: Attribute) => {
    if (!confirm(`Deactivate "${attr.name}"? Existing products keep the values already saved against it.`)) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            ...
            Define product attributes per category. Controls seller listing forms, storefront filters, and variant pickers.
          </p>
        </div>
        <button
          onClick={() => { setEditAttr(undefined); setModalCatId(null); setShowModal(true); }}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-2xl font-black text-slate-900">{groups.length}</p><p className="text-xs text-slate-500 mt-1">Categories Configured</p></div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-2xl font-black text-blue-600">{totalAttrs}</p><p className="text-xs text-slate-500 mt-1">Total Attributes</p></div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-2xl font-black text-emerald-600">{totalFilterable}</p><p className="text-xs text-slate-500 mt-1">Filterable</p></div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-2xl font-black text-purple-600">{totalVariantAxes}</p><p className="text-xs text-slate-500 mt-1">Variant Axes</p></div>
      </div>
      ...
      {attributesRes.error && !loading && <AdminErrorBanner error={attributesRes.error} onRetry={reload} />}
      ...
      {!loading && !attributesRes.error && filteredGroups.length === 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center">
          ...
          <p className="text-sm text-slate-500 mt-1 max-w-md mx-auto">
            Attributes drive the seller listing form, the storefront filters and the variant pickers.
            Start with a Colour and a Size on your largest category and mark both as variant axes.
          </p>
        </div>
      )}
          ...
          return (
            <div key={group.key} className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
              <div
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setExpanded(isExpanded ? null : group.key); } }}
              ...
              >
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); setModalCatId(group.categoryId); setEditAttr(undefined); setShowModal(true); }}
                      ...
                      {group.attributes.map((attr) => (
                        <tr key={attr.id} className="hover:bg-slate-50/50">
                          <td className="px-4 py-3">
                            ...
                            <p className="text-[11px] text-slate-400 font-mono">{attr.slug}{attr.unit ? ` · ${attr.unit}` : ''}</p>
                          </td>
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center gap-1.5 text-xs text-slate-600 bg-slate-100 px-2 py-1 rounded-lg">
                              {typeIcons[attr.type] ?? <Type className="w-3.5 h-3.5" />} {typeLabels[attr.type] ?? attr.type}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            {attr.options?.length ? (
                              <div className="flex flex-wrap gap-1 max-w-65">
                                {attr.options.slice(0, 5).map((o) => (
                                  <span key={o.value} className="inline-flex items-center gap-1 bg-slate-50 border border-slate-200 text-slate-600 text-[11px] px-1.5 py-0.5 rounded">
                                    {o.hex && <span className="w-2.5 h-2.5 rounded-full border border-slate-300" style={{ backgroundColor: o.hex }} />}
                                    {o.label}
                                  </span>
                                ))}
                                {attr.options.length > 5 && <span className="text-[11px] text-slate-400">+{attr.options.length - 5}</span>}
                              </div>
                            ) : <span className="text-xs text-slate-300">—</span>}
                          </td>
                          <td className="px-4 py-3 text-center">
                            {attr.isVariantAxis
                              ? <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-200 text-[11px] font-bold px-2 py-0.5 rounded-full"><Boxes className="w-3 h-3" /> Yes</span>
                              : <span className="text-xs text-slate-300">—</span>}
                          </td>
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center justify-center gap-1">
                              {attr.isRequired && <span className="text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded">REQ</span>}
                              {attr.isFilterable && <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded inline-flex items-center gap-0.5"><Filter className="w-2.5 h-2.5" />FILTER</span>}
                              {attr.isSearchable && <span className="text-[10px] font-bold text-blue-700 bg-blue-50 border border-blue-200 px-1.5 py-0.5 rounded">SEARCH</span>}
                            </div>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <button onClick={() => toggleActive(attr)} className="inline-flex items-center gap-1.5" aria-label={attr.isActive ? 'Deactivate' : 'Activate'}>
                              {attr.isActive
                                ? <><ToggleRight className="w-5 h-5 text-emerald-500" /><span className="text-xs text-emerald-600 font-bold">Active</span></>
                                : <><ToggleLeft className="w-5 h-5 text-slate-300" /><span className="text-xs text-slate-400 font-bold">Inactive</span></>}
                            </button>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-1">
                              <button onClick={() => { setEditAttr(attr); setModalCatId(attr.categoryId); setShowModal(true); }}
                                className="p-1.5 hover:bg-blue-50 rounded-lg" title="Edit" aria-label={`Edit ${attr.name}`}>
                                <Edit2 className="w-4 h-4 text-slate-400 hover:text-blue-600" />
                              </button>
                              <button onClick={() => remove(attr)}
                                className="p-1.5 hover:bg-red-50 rounded-lg" title="Deactivate" aria-label={`Deactivate ${attr.name}`}>
      ...
      {showModal && (
        <AttrModal
          ...
          saving={saving}
          onSave={save}
          onClose={() => { setShowModal(false); setEditAttr(undefined); }}
        />
      )}
