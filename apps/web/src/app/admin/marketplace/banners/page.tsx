...
import {
  useMarketplace,
  type HeroBanner,
  type ExtendedCampaignBanner,
  type ExtendedCountryBanner,
} from '@/lib/contexts/marketplace-context';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 bg-slate-50 border-b border-slate-200">
          ...
          <div
            className={`bg-linear-to-r ${gradient} rounded-xl h-48 flex items-center p-8 relative overflow-hidden`}
          >
            <div className="relative z-10 max-w-md">
              {tag && (
                <span className="bg-white/20 text-white text-[10px] font-bold px-3 py-1.5 mb-3 inline-block uppercase tracking-widest rounded-full">
                  {tag}
                </span>
              )}
              <h2 className="text-white text-2xl font-bold mb-3 leading-tight whitespace-pre-line">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="/marketplace/category/electronics"
            />
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span
                className={`text-xs font-bold ${status === 'active' ? 'text-emerald-600' : 'text-slate-400'}`}
              >
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Control homepage hero banners, campaign banners, and country-specific banners. Changes
      ...
      <div className="grid grid-cols-3 gap-3">
        {tabs.map((t) => {
          ...
          return (
            <div
              key={t.key}
              className={`bg-white border rounded-xl p-4 shadow-sm cursor-pointer transition-all ${activeTab === t.key ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200 hover:border-slate-300'}`}
              onClick={() => setActiveTab(t.key)}
              role="button"
              tabIndex={0}
              onKeyDown={activateOnKey(() => setActiveTab(t.key))}
            >
              <div className="flex items-center gap-2 mb-2">
                <Icon className="w-4 h-4 text-blue-600" />
                <span className="text-xs font-bold text-slate-500 uppercase">{t.label}</span>
              </div>
        ...
        <div className="space-y-3">
          {heroBanners
            ...
            .map((b, idx) => (
              <div
                key={b.id}
                className={`bg-white border rounded-2xl shadow-sm overflow-hidden transition-all ${b.status === 'inactive' ? 'opacity-60' : ''}`}
              >
                <div className="flex items-stretch">
                  {/* Preview */}
                  <div
                    className={`w-72 bg-linear-to-r ${b.gradient} p-5 shrink-0 flex flex-col justify-center relative overflow-hidden`}
                  >
                    {b.tag && (
                      <span className="bg-white/20 text-white text-[8px] font-bold px-2 py-1 uppercase tracking-widest rounded-full self-start mb-2">
                        {b.tag}
                      </span>
                    )}
                    <h3 className="text-white font-bold text-sm leading-tight whitespace-pre-line line-clamp-2">
                      {b.headline}
                    </h3>
                    <span className="bg-white text-slate-900 text-[9px] font-bold px-3 py-1 rounded mt-2 self-start">
                      {b.cta} →
                    </span>
                  </div>
                  ...
                  <div className="flex-1 p-4 flex items-center justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span
                          className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${b.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}
                        >
                      ...
                      <p className="text-xs text-slate-500 truncate">
                        CTA: <span className="font-mono text-blue-600">{b.ctaHref}</span>
                      </p>
                      {b.countries && (
                        <div className="flex gap-1 mt-1.5">
                          {b.countries.slice(0, 5).map((c) => (
                            <span
                              key={c}
                              className="text-[8px] bg-blue-50 text-blue-700 font-bold px-1.5 py-0.5 rounded"
                            >
                              {c}
                            </span>
                          ))}
                          {b.countries.length > 5 && (
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button
                        onClick={() => {
                          if (confirm('Delete this banner?')) deleteHeroBanner(b.id);
                        }}
              ...
              {campaignBanners.map((b) => (
                <tr key={b.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-4 py-3.5">
                    <div
                      className={`w-32 h-16 rounded-lg bg-linear-to-r ${b.gradient} flex items-center justify-center`}
                    >
                  ...
                  <td className="px-4 py-3.5 text-slate-600">
                    {b.headline}
                    <br />
                    <span className="text-xs text-slate-400">{b.subheadline}</span>
                  </td>
                  <td className="px-4 py-3.5">
                    <span className="text-xs font-mono text-blue-600">{b.ctaHref}</span>
                  </td>
                  <td className="px-4 py-3.5 text-center">
                    <button
                      onClick={() => toggleCampaignStatus(b.id)}
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-bold ${b.status === 'active' ? 'text-emerald-600' : 'text-slate-400'}`}
                      >
          ...
          {countryBanners.map((b) => (
            ...
            >
              ...
              <div className="flex border-t border-slate-100">
                <button
                  onClick={() => toggleCountryStatus(b.id)}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold hover:bg-slate-50 transition-colors"
                >
                  {b.status === 'active' ? (
                  ...
                  <span className={b.status === 'active' ? 'text-emerald-600' : 'text-slate-400'}>
