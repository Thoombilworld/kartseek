...
const CAMPAIGNS: Campaign[] = [
  ...
  },
  {
    ...
    orders: 0,
    revenue: 0,
    placement: ['Homepage', 'Category', 'Push'],
    country: 'India',
    conversionRate: 0,
  },
  {
    id: 'CMP-1004',
    name: 'Sponsored: Apple MacBook',
    ...
    orders: 320,
    revenue: 3680000,
    placement: ['Search Results', 'Category'],
    country: 'India',
    conversionRate: 3.6,
  },
  {
    ...
    type: 'Free Delivery Campaign',
    seller: 'Platform Campaign',
    products: 0,
    budget: 100000,
    spent: 92000,
    ...
    orders: 1240,
    revenue: 890000,
    placement: ['Homepage', 'Cart'],
    country: 'India',
    conversionRate: 10.3,
  },
  {
    id: 'CMP-1006',
    name: 'Clearance: Old Stock Fashion',
    type: 'Clearance Sale',
    seller: "Levi's India",
    products: 64,
    budget: 20000,
...
const TYPE_COLORS: Record<string, string> = {
  'Flash Sale': 'bg-red-50 text-red-700',
  'Festival Sale': 'bg-yellow-50 text-yellow-700',
  'Category Promotion': 'bg-indigo-50 text-indigo-700',
  'Sponsored Product': 'bg-blue-50 text-blue-700',
  'Free Delivery Campaign': 'bg-emerald-50 text-emerald-700',
  'Clearance Sale': 'bg-orange-50 text-orange-700',
  'Brand Promotion': 'bg-purple-50 text-purple-700',
};
      ...
      <div className="relative w-full max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-left">
        ...
        <div className="p-6 space-y-5">
          <div className="flex gap-2 flex-wrap">
            ...
            <span
              className={`text-[10px] font-bold px-2.5 py-1 rounded-md ${TYPE_COLORS[c.type] || 'bg-slate-100'}`}
            >
              {c.type}
            </span>
            {c.placement.map((p) => (
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl p-5 text-white">
            <div className="flex justify-between mb-2">
              <span className="text-sm font-bold opacity-80">Budget</span>
              <span className="text-sm font-bold">₹{c.budget.toLocaleString()}</span>
            </div>
            <div className="h-3 bg-white/20 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${budgetPct >= 90 ? 'bg-red-400' : budgetPct >= 60 ? 'bg-amber-300' : 'bg-emerald-300'}`}
                style={{ width: `${budgetPct}%` }}
              />
            </div>
            <div className="flex justify-between mt-2">
              <span className="text-xs opacity-70">Spent: ₹{c.spent.toLocaleString()}</span>
              <span className="text-xs opacity-70">{budgetPct}% used</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <p className="text-sm font-bold text-slate-900">{c.products || 'All'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Discount</p>
              <p className="text-sm font-bold text-emerald-600">{c.discount}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-[10px] text-blue-600">Impressions</p>
                </div>
                <div className="bg-purple-50 rounded-xl p-3 text-center">
                  <p className="text-lg font-black text-purple-700">{c.clicks.toLocaleString()}</p>
                  <p className="text-[10px] text-purple-600">Clicks</p>
                </div>
                <div className="bg-emerald-50 rounded-xl p-3 text-center">
                  <p className="text-lg font-black text-emerald-700">{c.orders.toLocaleString()}</p>
                  <p className="text-[10px] text-emerald-600">Orders</p>
                </div>
                <div className="bg-amber-50 rounded-xl p-3 text-center">
                  <p className="text-lg font-black text-amber-700">
                    ₹{(c.revenue / 100000).toFixed(1)}L
                  </p>
                  <p className="text-[10px] text-amber-600">Revenue</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3 mt-3">
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <p className="text-sm font-black text-slate-700">{ctr}%</p>
                  <p className="text-[10px] text-slate-500">CTR</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <p className="text-sm font-black text-slate-700">{c.conversionRate}%</p>
                  <p className="text-[10px] text-slate-500">Conv. Rate</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <p className="text-sm font-black text-slate-700">
                    ₹{c.orders ? Math.round(c.spent / c.orders) : 0}
                  </p>
              ...
              <div className="mt-3 space-y-1">
                {[
                  { l: 'Impressions', v: c.impressions, c: 'bg-blue-500' },
                  { l: 'Clicks', v: c.clicks, c: 'bg-purple-500' },
                  { l: 'Orders', v: c.orders, c: 'bg-emerald-500' },
                ].map((f) => (
                  <div key={f.l} className="flex items-center gap-2">
                    <span className="text-[10px] text-slate-500 w-20">{f.l}</span>
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${f.c}`}
                        style={{ width: `${c.impressions ? (f.v / c.impressions) * 100 : 0}%` }}
                      />
                    </div>
                    <span className="text-[10px] font-bold text-slate-700 w-16 text-right">
                      {f.v.toLocaleString()}
                    </span>
                  </div>
                ))}
          ...
          <div className="space-y-3">
            {c.status === 'Pending Approval' && (
              <div className="flex gap-3">
                ...
                <button
                  onClick={onReject}
                  className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2"
                >
            ...
            {c.status === 'Active' && (
              <button
                onClick={onPause}
                className="w-full bg-amber-50 hover:bg-amber-100 text-amber-700 py-3 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2"
              >
...
function CampaignModal({
  ...
  const [form, setForm] = useState({
    name: campaign?.name || '',
    type: campaign?.type || 'Flash Sale',
    seller: campaign?.seller || '',
    products: campaign?.products || 0,
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="campaign-name">
              Campaign Name
            </label>
            <input
              id="campaign-name"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              >
                ...
                <option>Free Delivery Campaign</option>
                <option>Clearance Sale</option>
                <option>Brand Promotion</option>
              </select>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="seller">
                Seller
              </label>
              <input
                id="seller"
                value={form.seller}
                onChange={(e) => u('seller', e.target.value)}
                className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-200"
                placeholder="Seller or 'Multiple'"
              />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="products">
                Products
              </label>
              <input
                id="products"
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="budget">
                Budget (₹)
              </label>
              <input
                id="budget"
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="discount">
                Discount
              </label>
              <input
                id="discount"
            ...
            <div className="flex flex-wrap gap-2">
              {PLACEMENTS.map((p) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-xl border transition-colors ${form.placement.includes(p) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-blue-50'}`}
                >
                  {p}
                </button>
              ))}
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <select
          ...
          className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm mb-3 outline-none"
        >
          <option>Discount exceeds maximum allowed</option>
          <option>Invalid campaign dates</option>
          <option>Misleading campaign name</option>
          <option>Prohibited product category</option>
          <option>Budget insufficient for placement</option>
          <option>Seller compliance issue</option>
          <option>Other</option>
        </select>
        <textarea
          value={notes}
          ...
          rows={3}
          className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm resize-none outline-none mb-4"
        />
        <div className="flex gap-3">
          <button
            onClick={onClose}
          ...
          <button
            onClick={() => onReject(`${reason}${notes ? ` — ${notes}` : ''}`)}
...
export default function CampaignsPage() {
  ...
  const {
    ...
    toast,
    showToast,
  } = useAdminData(() => adminMarketplaceApi.getCampaigns({ country }), [country]);
  ...
  const {
    ...
    regionLabel,
    isFiltered,
  } = useMarketplaceRegionFilter(CAMPAIGNS);
  ...
  const handleApprove = (c: Campaign) => {
    execute(
      () => adminMarketplaceApi.updateCampaign(c.id, { status: 'Approved' }),
      `${c.name} approved`,
      () => refetch(),
    );
    setSelected(null);
  };
  const handleReject = (c: Campaign, reason: string) => {
    execute(
      () => adminMarketplaceApi.updateCampaign(c.id, { status: 'Rejected', reason }),
      `${c.name} rejected`,
      () => refetch(),
    );
    setShowReject(null);
  ...
  const handlePause = (c: Campaign) => {
    execute(
      () => adminMarketplaceApi.updateCampaign(c.id, { status: 'Paused' }),
      `${c.name} paused`,
      () => refetch(),
    );
    setSelected(null);
  ...
  const handleResume = (c: Campaign) => {
    execute(
      () => adminMarketplaceApi.updateCampaign(c.id, { status: 'Active' }),
      `${c.name} resumed`,
      () => refetch(),
    );
    setSelected(null);
  ...
  const handleCreate = (data: any) => {
    execute(
      () => adminMarketplaceApi.createCampaign(data),
      'Campaign created',
      () => refetch(),
    );
    setShowCreate(false);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {isFiltered ? `${regionLabel} — ` : ''}Create, approve, and monitor marketing campaigns
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl p-5 text-white">
          <p className="text-sm font-bold opacity-80">Total Budget</p>
          <p className="text-2xl font-black mt-1">₹{(totalBudget / 100000).toFixed(1)}L</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-slate-500">Spent</p>
          <p className="text-xl font-black text-amber-600">₹{(totalSpent / 100000).toFixed(1)}L</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        ...
        <div className="bg-white border border-amber-200 rounded-xl p-4 shadow-sm">
          ...
          <p className="text-xl font-black text-amber-600">
            {regionFiltered.filter((c) => c.status === 'Pending Approval').length}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search campaign, seller, or ID..."
        ...
        <div className="flex gap-2 flex-wrap">
          {['All', 'Active', 'Pending Approval', 'Scheduled', 'Paused', 'Expired', 'Rejected'].map(
            (s) => (
              <button
                key={s}
                onClick={() => {
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {paged.length === 0 ? (
              <tr>
                <td colSpan={7}>
                  <MarketplaceEmptyState title="No campaigns found" icon={Megaphone} />
                </td>
              </tr>
            ) : (
              paged.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${TYPE_COLORS[c.type] || 'bg-slate-100'}`}
                      >
                        {c.type}
                      </span>
                    </td>
                    <td className="px-4 py-3.5 text-center">
                      ...
                      <div className="h-1.5 bg-slate-100 rounded-full mt-1 w-16 mx-auto overflow-hidden">
                        <div
                          className={`h-full rounded-full ${budgetPct >= 90 ? 'bg-red-500' : 'bg-blue-500'}`}
                          style={{ width: `${budgetPct}%` }}
                        />
                      </div>
                    </td>
                    <td className="px-4 py-3.5 text-center text-xs text-slate-600">
                      {c.start}
                      <br />
                      <span className="text-slate-400">→ {c.end}</span>
                    </td>
                    <td className="px-4 py-3.5 text-center">
                      {c.impressions > 0 ? (
                        <>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        {c.status === 'Pending Approval' && (
                          <>
                            <button
                              ...
                              className="p-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg"
                            >
                        ...
                        {c.status === 'Active' && (
                          <button
                            ...
                            className="p-1 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-lg"
                          >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2">
          <p className="text-xs text-slate-500">{filtered.length} campaigns</p>
      ...
      {showReject && (
        <RejectModal
          onReject={(reason) => handleReject(showReject, reason)}
