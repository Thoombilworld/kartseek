...
type CommissionRule = {
  ...
  value: string;
  tiers?: { min: number; max: number; rate: string }[];
  effective: string;
  expiry: string | null;
...
const COMMISSIONS: CommissionRule[] = [
  {
    ...
    tiers: [
      ...
      { min: 10001, max: 50000, rate: '6%' },
      { min: 50001, max: 999999, rate: '5%' },
    ],
    effective: '2026-01-01',
    expiry: '2026-12-31',
    status: 'active',
    totalEarned: 4520000,
    ordersAffected: 12400,
    history: [
      { date: '2026-01-01', oldValue: '8%', newValue: '5-8% (Tiered)', changedBy: 'Finance Admin' },
      { date: '2025-07-01', oldValue: '10%', newValue: '8%', changedBy: 'Raj (Admin)' },
    ],
  },
  {
    id: 'COM-02',
    ...
    status: 'active',
    totalEarned: 3200000,
    ordersAffected: 18600,
    history: [{ date: '2026-01-01', oldValue: '12%', newValue: '14%', changedBy: 'Finance Admin' }],
  },
  {
    id: 'COM-03',
    seller: 'All Sellers',
  ...
  },
  {
    ...
    totalEarned: 1800000,
    ordersAffected: 4200,
    history: [{ date: '2026-03-01', oldValue: '5%', newValue: '3%', changedBy: 'Raj (Admin)' }],
  },
  {
    id: 'COM-06',
    seller: 'All Sellers',
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl overflow-y-auto animate-slide-left">
        ...
        <div className="p-6 space-y-5">
          <div className="flex gap-2">
            <span
              className={`text-[10px] font-bold px-2.5 py-1 rounded-md ${STATUS_STYLES[r.status]}`}
            >
              {r.status}
            </span>
          ...
          <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-xl p-5 text-white text-center">
            <p className="text-sm font-bold opacity-80">Commission Rate</p>
            <p className="text-4xl font-black mt-1">{r.value}</p>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="px-3 py-2 text-left text-slate-500">Order Value Range</th>
                    ...
                    {r.tiers.map((t, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0">
                        <td className="px-3 py-2 font-medium text-slate-700">
                          {formatCurrency(t.min)} — {formatCurrency(t.max)}
                        </td>
                        <td className="px-3 py-2 text-right font-black text-purple-700">
                          {t.rate}
                        </td>
                      </tr>
                    ))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-emerald-50 rounded-xl p-3 text-center">
              <p className="text-lg font-black text-emerald-700">{formatCurrency(r.totalEarned)}</p>
              <p className="text-[10px] text-emerald-600">Total Earned</p>
            </div>
            <div className="bg-blue-50 rounded-xl p-3 text-center">
              <p className="text-lg font-black text-blue-700">
                {r.ordersAffected.toLocaleString()}
              </p>
              <p className="text-[10px] text-blue-600">Orders Affected</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Effective</p>
              <p className="text-sm font-bold text-slate-900">{r.effective}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Expiry</p>
              <p className="text-sm font-bold text-slate-900">{r.expiry || 'No expiry'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
            ...
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Avg. per Order</p>
              <p className="text-sm font-bold text-slate-900">
                {r.ordersAffected
                  ? formatCurrency(Math.round(r.totalEarned / r.ordersAffected))
                  : '—'}
              </p>
            </div>
          </div>
              ...
              <div className="space-y-0 relative">
                ...
                {r.history.map((h, i) => (
                  <div key={i} className="flex items-start gap-3 py-2 relative">
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 z-10 ${i === 0 ? 'bg-purple-600' : 'bg-slate-200'}`}
                    >
                      <Clock className={`w-3 h-3 ${i === 0 ? 'text-white' : 'text-slate-500'}`} />
                    </div>
                    <div>
                      <p className="text-xs text-slate-900">
                        <span className="font-bold text-red-500 line-through">{h.oldValue}</span> →{' '}
                        <span className="font-bold text-emerald-600">{h.newValue}</span>
                      </p>
...
function RuleModal({
  ...
  const [form, setForm] = useState({
    seller: rule?.seller || 'All Sellers',
    ...
    expiry: rule?.expiry || '',
    tiers: rule?.tiers || [
      { min: 0, max: 10000, rate: '' },
      { min: 10001, max: 50000, rate: '' },
      { min: 50001, max: 999999, rate: '' },
    ],
  });
  const u = (k: string, v: any) => setForm((f) => ({ ...f, [k]: v }));
  const updateTier = (i: number, field: string, val: any) => {
    const t = [...form.tiers];
    (t[i] as any)[field] = val;
    setForm((f) => ({ ...f, tiers: t }));
  };
  const addTier = () =>
    setForm((f) => ({ ...f, tiers: [...f.tiers, { min: 0, max: 0, rate: '' }] }));
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800 mb-4">
          <strong>⚠️</strong> Changes are permanently logged in the Audit Trail with old value, new
          value, and admin name.
        </div>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="category">
                Category
              </label>
              <input
                id="category"
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="country">
                Country
              </label>
              <select
                id="country"
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none"
              placeholder="All Sellers or specific"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            {form.type !== 'Tiered' && (
              <div>
                <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="value">
                  Value
                </label>
                <input
                  id="value"
                  ...
                  className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-blue-200"
                  placeholder={form.type === 'Percentage' ? 'e.g. 8%' : 'e.g. ₹15/order'}
                />
              </div>
            )}
          </div>
                  ...
                  <div key={i} className="flex items-center gap-2 bg-slate-50 rounded-xl p-3">
                    <div className="flex-1 grid grid-cols-3 gap-2">
                      ...
                      <input
                        ...
                        className="border border-slate-200 rounded-lg p-2 text-xs outline-none font-bold"
                        placeholder="Rate %"
                      />
                    </div>
                    <button
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => u('effective', e.target.value)}
                className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="expiry-date">
                Expiry Date
              </label>
              <input
                id="expiry-date"
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            onClick={() => onSave(form)}
            disabled={!form.category || !form.effective}
...
export default function CommissionsPage() {
  ...
  const {
    ...
    toast,
    showToast,
  } = useAdminData(() => adminMarketplaceApi.getCommissions(country), [country]);
  ...
  const {
    ...
    isFiltered,
    formatCurrencyValue,
  } = useMarketplaceRegionFilter(COMMISSIONS);
  ...
  const handleSave = (data: any) => {
    execute(
      () => adminMarketplaceApi.updateCommission('new', data),
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Commission Rules</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            {isFiltered ? `${regionLabel} — ` : ''}Configure category-level and seller-specific
            commission rates with tiered pricing
          </p>
        </div>
        <div className="flex gap-2">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800 flex items-start gap-2">
        ...
        <span>
          <strong>⚠️ Audit-Logged:</strong> All commission changes are permanently logged with admin
          name, old value, new value, and timestamp.
        </span>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-xl p-5 text-white">
          <p className="text-sm font-bold opacity-80">Total Earned</p>
          <p className="text-2xl font-black mt-1">{fmt(totalEarned)}</p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-900">
            {new Set(regionFiltered.map((c) => c.category)).size}
          </p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-slate-500">Tiered Rules</p>
          <p className="text-2xl font-black text-purple-600">
            {regionFiltered.filter((c) => c.type === 'Tiered').length}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search category or seller..."
        ...
        <div className="flex gap-2">
          {['all', 'active', 'scheduled', 'expired'].map((s) => (
            <button
              ...
              className={`px-3 py-2 text-xs font-bold rounded-xl border transition-colors capitalize ${statusFilter === s ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
            >
              {s === 'all' ? 'All' : s}
            </button>
          ))}
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8}>
                  <MarketplaceEmptyState title="No commission rules found" icon={Percent} />
                </td>
              </tr>
            ) : (
              filtered.map((c) => (
                ...
                >
                  <td className="px-4 py-3.5">
                    <p className="font-bold text-slate-900 text-xs">{c.category}</p>
                    <p className="text-[10px] text-slate-400">
                      {c.id} · <CountryFlag code={COUNTRY_TO_CODE[c.country] || 'IN'} size="sm" />{' '}
                      {c.country}
                    </p>
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${c.type === 'Tiered' ? 'bg-purple-50 text-purple-700' : c.type === 'Flat' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'}`}
                    >
                      {c.type}
                    </span>
                  </td>
                  <td className="px-4 py-3.5 text-center font-black text-lg text-purple-700">
                    {c.value}
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-right text-xs text-slate-600">
                    {c.ordersAffected.toLocaleString()}
                  </td>
                  <td className="px-4 py-3.5 text-center">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md capitalize ${STATUS_STYLES[c.status]}`}
                    >
                      {c.status}
                    </span>
                  </td>
      ...
      {selected && (
        <CommissionDrawer rule={selected} onClose={() => setSelected(null)} formatCurrency={fmt} />
      )}
