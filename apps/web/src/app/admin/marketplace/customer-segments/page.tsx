...
const SEGMENTS: Segment[] = [
  {
    ...
    description: 'Customers who have spent over ₹50,000 lifetime',
    country: 'India',
    type: 'Value-Based',
    customerCount: 2840,
    avgOrderValue: 12500,
    ...
    conversionRate: 8.2,
    rules: [{ field: 'lifetime_spend', operator: '>=', value: '50000' }],
  ...
  },
  {
    id: 'SEG-002',
    name: 'Repeat Buyers',
    description: 'Placed 3+ orders in last 90 days',
    country: 'India',
    type: 'Behavioral',
    customerCount: 5200,
    avgOrderValue: 4800,
    totalRevenue: 24960000,
    conversionRate: 12.5,
    rules: [{ field: 'orders_90d', operator: '>=', value: '3' }],
    lastUpdated: '2026-06-06',
    isActive: true,
    autoSync: true,
  },
  {
    ...
    description: "Added to cart but didn't checkout in 7 days",
    country: 'India',
    type: 'Behavioral',
    customerCount: 8400,
    avgOrderValue: 3200,
    ...
    conversionRate: 0,
    rules: [
      { field: 'abandoned_cart_7d', operator: '=', value: 'true' },
  ...
  },
  {
    ...
    conversionRate: 3.8,
    rules: [{ field: 'signup_days', operator: '<=', value: '30' }],
    lastUpdated: '2026-06-06',
    isActive: true,
    autoSync: true,
  },
  {
    ...
    description: 'No orders in last 180 days',
    country: 'India',
    type: 'Lifecycle',
    customerCount: 18000,
    avgOrderValue: 0,
    totalRevenue: 0,
    conversionRate: 0.5,
    rules: [{ field: 'last_order_days', operator: '>', value: '180' }],
  ...
  },
  {
    id: 'SEG-006',
    name: 'UAE Premium',
    description: 'UAE customers with avg order > AED 1000',
    country: 'UAE',
    type: 'Value-Based',
    customerCount: 420,
    avgOrderValue: 28000,
    ...
    conversionRate: 6.4,
    rules: [
      { field: 'country', operator: '=', value: 'UAE' },
      { field: 'avg_order', operator: '>=', value: '1000' },
    ],
  ...
  },
  {
    ...
    description: 'Bought 2+ electronics items',
    country: 'India',
    type: 'Behavioral',
    customerCount: 9500,
    avgOrderValue: 15000,
    ...
    conversionRate: 7.1,
    rules: [{ field: 'category_electronics', operator: '>=', value: '2' }],
  ...
  {
    id: 'SEG-008',
    name: 'COD-Only Buyers',
    description: 'All orders paid via Cash on Delivery',
    country: 'India',
    type: 'Behavioral',
    customerCount: 14200,
    avgOrderValue: 1800,
    ...
    conversionRate: 4.2,
    rules: [
      { field: 'payment_method', operator: '=', value: 'COD' },
      { field: 'online_payment', operator: '=', value: 'never' },
    ],
...
const TYPE_STYLES: Record<string, string> = {
  ...
  Demographic: 'bg-purple-50 text-purple-700',
  'Value-Based': 'bg-emerald-50 text-emerald-700',
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl overflow-y-auto animate-slide-left">
        ...
        <div className="p-6 space-y-5">
          <div className="flex gap-2">
            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md ${TYPE_STYLES[s.type]}`}>
              {s.type}
            </span>
            <span
              className={`text-[10px] font-bold px-2 py-1 rounded-md ${s.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}
            >
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-xl p-5 text-white text-center">
            <p className="text-sm font-bold opacity-80">Customers in Segment</p>
            <p className="text-4xl font-black mt-1">{s.customerCount.toLocaleString()}</p>
            <p className="text-xs opacity-60 mt-1">
              <CountryFlag code={COUNTRY_TO_CODE[s.country] || 'UN'} size="sm" /> {s.country}
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-emerald-50 rounded-xl p-3 text-center">
              <p className="text-lg font-black text-emerald-700">{fmt(s.avgOrderValue)}</p>
              <p className="text-[10px] text-emerald-600">Avg. Order Value</p>
            </div>
            <div className="bg-blue-50 rounded-xl p-3 text-center">
              <p className="text-lg font-black text-blue-700">{s.conversionRate}%</p>
              <p className="text-[10px] text-blue-600">Conversion Rate</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3 text-center">
              <p className="text-lg font-black text-slate-900">{fmt(s.totalRevenue)}</p>
              <p className="text-[10px] text-slate-500">Total Revenue</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3 text-center">
              <p className="text-lg font-black text-slate-900">{s.lastUpdated}</p>
              <p className="text-[10px] text-slate-500">Last Updated</p>
            </div>
          </div>

          <div>
            <h3 className="text-sm font-bold text-slate-900 mb-2">Segment Rules</h3>
          ...
          <div className="flex gap-3">
            <button className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2">
              <Target className="w-4 h-4" /> Target with Campaign
            </button>
...
function CreateSegmentModal({
  ...
  const updateRule = (i: number, field: string, val: string) => {
    const r = [...form.rules];
    (r[i] as any)[field] = val;
    setForm((f) => ({ ...f, rules: r }));
  };
  const addRule = () =>
    setForm((f) => ({ ...f, rules: [...f.rules, { field: '', operator: '>=', value: '' }] }));
  const FIELDS = [
    'lifetime_spend',
    'orders_90d',
    'last_order_days',
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="name">
              Name
            </label>
            <input
              id="name"
            ...
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-2 text-xs font-bold text-slate-600 cursor-pointer">
                <input
                  ...
                  onChange={(e) => u('autoSync', e.target.checked)}
          ...
          <div>
            <label className="text-xs font-bold text-slate-600 mb-2 block">Rules</label>
            {form.rules.map((r, i) => (
              <div key={i} className="flex gap-2 mb-2">
                ...
                <select
                  ...
                  className="w-16 border border-slate-200 rounded-lg p-2 text-xs outline-none"
                >
                  <option>=</option>
                  <option>{'>'}</option>
                  <option>{'<'}</option>
                ...
                <input
                  ...
                  className="w-24 border border-slate-200 rounded-lg p-2 text-xs outline-none"
                  placeholder="Value"
                />
              </div>
            ))}
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            onClick={() => onSave(form)}
            disabled={!form.name}
            className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white py-2.5 rounded-xl text-sm font-bold"
          >
            Create
          </button>
        </div>
...
export default function CustomerSegmentsPage() {
  ...
  const {
    ...
    toast,
    showToast,
  } = useAdminData(() => adminMarketplaceApi.getCustomers({ country }), [country]);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Customer Segments</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            Rule-based customer segmentation for targeted campaigns
          </p>
        </div>
        <div className="flex gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-xl p-5 text-white">
          <p className="text-sm font-bold opacity-80">Total Segmented</p>
          <p className="text-2xl font-black mt-1">{totalCustomers.toLocaleString()}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-slate-500">Active Segments</p>
          <p className="text-xl font-black text-emerald-600">
            {regionFiltered.filter((s) => s.isActive).length}
          </p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-slate-500">Auto-Syncing</p>
          <p className="text-xl font-black text-blue-600">
            {regionFiltered.filter((s) => s.autoSync).length}
          </p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-slate-500">Segment Types</p>
          <p className="text-xl font-black text-slate-900">
            {new Set(regionFiltered.map((s) => s.type)).size}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {['all', 'Behavioral', 'Value-Based', 'Lifecycle', 'Custom'].map((s) => (
            <button
              key={s}
              onClick={() => setFilter(s)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.length === 0 ? (
          <MarketplaceEmptyState title="No segments found" icon={Users} />
        ) : (
          filtered.map((s) => (
            <div
              key={s.id}
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1">
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${TYPE_STYLES[s.type]}`}
                  >
                    {s.type}
                  </span>
                  {!s.isActive && (
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="text-center">
                  <p className="text-lg font-black text-emerald-700">{fmt(s.avgOrderValue)}</p>
                  <p className="text-[10px] text-slate-500">Avg. Order</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-black text-blue-700">{s.conversionRate}%</p>
              ...
              <div className="flex items-center gap-2 mt-3 text-[10px] text-slate-400">
                ...
                {s.autoSync && <span className="text-blue-600 font-bold">· Auto-sync</span>}
              </div>
            </div>
          ))
        )}
