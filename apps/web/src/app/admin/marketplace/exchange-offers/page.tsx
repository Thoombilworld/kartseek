...
import { ArrowLeftRight, Plus, Eye, Pause, Play, Trash2, Star, Smartphone, Laptop, Clock, TrendingUp, Package, Truck } from 'lucide-react';
...
const EXCHANGE_OFFERS = [
  {
    ...
    fulfillmentMode: 'PICKUP', startsAt: '01 Jun 2026', expiresAt: '31 Jul 2026', priority: 1,
  ...
  },
  {
    ...
    fulfillmentMode: 'PICKUP', startsAt: '15 Jun 2026', expiresAt: '15 Aug 2026', priority: 2,
  ...
  },
  {
    ...
    fulfillmentMode: 'PICKUP', startsAt: '01 Jul 2026', expiresAt: '31 Aug 2026', priority: 3,
  ...
  },
  {
    ...
    fulfillmentMode: 'PICKUP', startsAt: '01 Apr 2026', expiresAt: '31 May 2026', priority: 4,
  ...
  },
  {
    ...
    fulfillmentMode: 'DROP_OFF', startsAt: '10 Jun 2026', expiresAt: '10 Aug 2026', priority: 5,
    status: 'Active', isFeatured: false, totalExchanges: 680, country: 'India',
  },
];
...
export default function ExchangeOffersPage() {
  ...
  const { data: apiData, loading, error, refetch, toast, showToast } = useAdminData(
      () => adminMarketplaceApi.getExchangeOffers(),
      []
    );
    const { execute } = useAdminAction(showToast);


  const { filtered: regionFiltered, regionLabel, isFiltered: isRegionFiltered } = useMarketplaceRegionFilter(EXCHANGE_OFFERS);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Total Exchanges', count: totalExchanges.toLocaleString('en-IN'), color: 'bg-teal-100 text-teal-600', Icon: TrendingUp },
          { label: 'Categories', count: new Set(regionFiltered.map(o => o.targetCategory)).size, color: 'bg-purple-100 text-purple-600', Icon: Package },
      ...
      <div className="flex gap-2 flex-wrap">
        {['All', 'Active', 'Paused', 'Draft', 'Expired', 'Archived'].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-2 text-xs font-bold rounded-xl border transition-colors ${statusFilter === s ? 'bg-teal-600 text-white border-teal-600' : 'bg-white text-slate-600 border-slate-200 hover:border-teal-300'}`}>{s}</button>
        ))}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(o => {
                const fulfillment = FULFILLMENT_STYLES[o.fulfillmentMode] || { label: o.fulfillmentMode, color: 'bg-slate-100 text-slate-600' };
                return (
                  <tr key={o.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 text-sm">
                        <span className="text-xs font-bold bg-red-50 text-red-700 px-2 py-0.5 rounded-lg">{o.exchangeCategory}</span>
                        <ArrowLeftRight className="w-3 h-3 text-slate-400" />
                        <span className="text-xs font-bold bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-lg">{o.targetCategory}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-center">
                      <span className={`text-xs font-bold px-2 py-1 rounded-lg ${fulfillment.color}`}>{fulfillment.label}</span>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex gap-1 flex-wrap max-w-[140px]">
                        {o.applicableBrandIds.slice(0, 3).map(b => (
                          <span key={b} className="text-[9px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-bold">{b}</span>
                        ))}
                        {o.applicableBrandIds.length > 3 && (
                          <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-bold">+{o.applicableBrandIds.length - 3}</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-center">
                      <p className="text-lg font-black text-slate-800">{o.totalExchanges.toLocaleString('en-IN')}</p>
                    </td>
                    <td className="px-5 py-4 text-center">
          ...
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea id="description" rows={2} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="Short description for product pages..." />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="exchange-category">Exchange Category *</label>
                  <input id="exchange-category" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="Smartphones" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="target-category">Target Category *</label>
                  <input id="target-category" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="Smartphones" />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="max-value">Max Value *</label>
                  <input id="max-value" type="number" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="15000" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="min-value">Min Value</label>
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="fulfillment-mode">Fulfillment Mode</label>
                <select id="fulfillment-mode" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                  <option value="PICKUP">Home Pickup</option><option value="DROP_OFF">Drop Off</option><option value="COURIER">Courier</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="start-date">Start Date *</label>
                  <input id="start-date" type="date" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"  aria-label="date"/>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="end-date">End Date *</label>
                  <input id="end-date" type="date" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"  aria-label="date"/>
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-5">
              ...
              <button className="flex-1 bg-teal-600 hover:bg-teal-700 text-white font-bold py-2.5 rounded-xl text-sm transition-colors">Create Offer</button>
            </div>
          </div>
        </div>
      )}
