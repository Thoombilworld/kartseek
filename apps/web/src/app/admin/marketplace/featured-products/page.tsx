...
import {
  Zap, Tag, Sparkles, Crown, TrendingUp, Heart, Target,
  Plus, X, Trash2, GripVertical, Calendar, ChevronDown, ChevronUp,
  Star, Package, Search, Save, Eye, EyeOff, ToggleLeft, ToggleRight,
...
const SECTION_META: Record<string, { icon: React.ElementType; color: string; accent: string; description: string }> = {
  'flash-deals': { icon: Zap, color: 'text-red-500', accent: 'bg-red-50 border-red-200', description: 'Time-limited deals with countdown timer on homepage' },
  'deals-of-day': { icon: Tag, color: 'text-blue-600', accent: 'bg-blue-50 border-blue-200', description: 'Handpicked daily deals, refreshed every 24 hours' },
  'new-arrivals': { icon: Sparkles, color: 'text-cyan-600', accent: 'bg-cyan-50 border-cyan-200', description: 'Recently added products highlighted on homepage' },
  'best-sellers': { icon: Crown, color: 'text-amber-600', accent: 'bg-amber-50 border-amber-200', description: 'Most purchased products this month' },
  'trending': { icon: TrendingUp, color: 'text-violet-600', accent: 'bg-violet-50 border-violet-200', description: 'Most searched and viewed products right now' },
  'recommended': { icon: Heart, color: 'text-pink-600', accent: 'bg-pink-50 border-pink-200', description: 'AI-recommended products based on browsing trends' },
  'sponsored': { icon: Target, color: 'text-slate-600', accent: 'bg-slate-50 border-slate-200', description: 'Paid placements from seller advertising' },
};
...
function ProductPickerModal({ section, existingIds, onAdd, onClose }: {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res: any = await adminMarketplaceApi.getProducts({ search: search || undefined, limit: 40 });
        if (cancelled) return;
        const rows: any[] = Array.isArray(res?.data) ? res.data : (res?.data?.data ?? []);
        setPool(rows.map((p: any) => ({
          id: p.id,
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col">
        ...
        <div className="px-5 pt-4">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products by name or brand..." className="w-full border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-5 space-y-2">
          {filtered.slice(0, 30).map(p => {
            ...
            return (
              <button key={p.id} onClick={() => toggle(p)} className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${isSel ? 'border-blue-500 bg-blue-50' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${isSel ? 'border-blue-500 bg-blue-500' : 'border-slate-300'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {p.mrp > p.price && <span className="text-[10px] text-green-600 font-bold">{discountPercent(p.mrp, p.price)}% off</span>}
                    <span className="bg-green-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-0.5">{p.rating} <Star className="w-2 h-2 fill-white" /></span>
                  </div>
                </div>
              </button>
            );
          ...
          {/* Three distinct states, because the search is a network call now:
              ...
              as an empty one. */}
          {searching && <p className="text-center text-slate-400 text-sm py-8">Searching the catalogue…</p>}
          {!searching && searchError && (
            <p role="alert" className="text-center text-red-600 text-sm py-8">{searchError}</p>
          )}
...
export default function FeaturedProductsPage() {
  ...
  const { data: apiData, loading, error, refetch, toast, showToast } = useAdminData(
    () => adminMarketplaceApi.getFeaturedProducts(),
    []
  );
  ...
  const removeProduct = (sectionKey: string, productId: string) => {
    ...
    if (section) {
      updateFeaturedSection(sectionKey, { products: section.products.filter(p => p.id !== productId) });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        {featuredSections.map(s => {
          ...
          return (
            <button key={s.key} onClick={() => setExpandedSection(expandedSection === s.key ? null : s.key)}
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase leading-tight">{s.label}</p>
            </button>
          );
        })}
      </div>
      ...
      {featuredSections.map(section => {
        ...
        return (
          <div key={section.key} className={`bg-white border rounded-2xl shadow-sm overflow-hidden transition-all ${isExpanded ? 'border-blue-200' : 'border-slate-200'}`}>
            {/* Header */}
            <button onClick={() => setExpandedSection(isExpanded ? null : section.key)}
              ...
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl ${meta?.accent || 'bg-slate-50'} flex items-center justify-center border`}>
              ...
              <div className="flex items-center gap-3">
                {section.isScheduled && section.startDate && (
                  <span className="text-[9px] text-blue-600 font-bold bg-blue-50 px-2 py-1 rounded-full flex items-center gap-1"><Clock className="w-3 h-3" /> Scheduled</span>
                )}
              ...
              <div className="border-t border-slate-100">
                ...
                <div className="flex items-center justify-between px-5 py-3 bg-slate-50/50">
                  <div className="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    {section.products.length} product{section.products.length !== 1 ? 's' : ''} in this section
                  </div>
                  <button onClick={() => setPickerSection(section.key)}
                ...
                <div className="divide-y divide-slate-100">
                  {section.products.map((product, idx) => {
                    ...
                    return (
                      <div key={product.id} className="flex items-center gap-4 px-5 py-3 hover:bg-slate-50/50 transition-colors group">
                        ...
                        {product.badge && (
                          <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full text-white ${product.badgeColor || 'bg-slate-600'}`}>{product.badge}</span>
                        )}
                        <button onClick={() => removeProduct(section.key, product.id)}
                          className="p-1.5 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity" title="Remove">
                ...
                {section.products.length === 0 && (
                  <div className="p-8 text-center">
                    ...
                    <button onClick={() => setPickerSection(section.key)} className="mt-2 text-blue-600 text-sm font-bold">+ Add Products</button>
                  </div>
                )}
              </div>
            )}
      ...
      {pickerSection && (
        <ProductPickerModal
          ...
          existingIds={featuredSections.find(s => s.key === pickerSection)?.products.map(p => p.id) || []}
