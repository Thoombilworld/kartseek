...
const HOURLY_DATA = [
  { hour: '6AM', sales: 12400 },
  { hour: '7AM', sales: 28900 },
  { hour: '8AM', sales: 45200 },
  { hour: '9AM', sales: 67800 },
  { hour: '10AM', sales: 89400 },
  { hour: '11AM', sales: 124000 },
  { hour: '12PM', sales: 156000 },
  { hour: '1PM', sales: 134000 },
  { hour: '2PM', sales: 98000 },
  { hour: '3PM', sales: 112000 },
  { hour: '4PM', sales: 145000 },
  { hour: '5PM', sales: 167000 },
];
...
const fmtDate = (v: unknown) => (v ? String(v).slice(0, 16).replace('T', ' ') : '');

/**
 * A campaign row as the API returns it (`flash_deals`) → the shape this page
 * renders. Statuses arrive upper-case; the style maps are keyed lower-case,
 * and an unknown key used to throw on `.bg` and take the whole page down.
 */
function toDealRow(api: any): FlashDeal & { region?: string } {
  const raw = String(api.status ?? '').toUpperCase();
  const now = Date.now();
  const end = api.windowEnd ? new Date(api.windowEnd).getTime() : 0;
  const start = api.windowStart ? new Date(api.windowStart).getTime() : 0;
  const status: DealStatus =
    raw === 'PAUSED'
      ? 'paused'
      : raw === 'CANCELLED' || raw === 'ENDED' || (end && end < now)
        ? 'ended'
        : raw === 'SCHEDULED' || (start && start > now)
          ? 'scheduled'
          : 'active';
  const nominations = Array.isArray(api.nominations) ? api.nominations : [];
  return {
    id: String(api.id),
    name: String(api.name ?? ''),
    products: nominations.length,
    discount: api.minDiscountPercent ? `${api.minDiscountPercent}%+` : '—',
    stockLimit: Number(api.stockLimit ?? 0) || 0,
    sold: Number(api.unitsSold ?? 0) || 0,
    revenue: 0,
    orders: Number(api.unitsSold ?? 0) || 0,
    views: 0,
    start: fmtDate(api.windowStart),
    end: fmtDate(api.windowEnd),
    status,
    priority: Number(api.priority ?? 0) || 0,
...
/** A nomination row as the API returns it (with deal, product, seller) → the page's shape. */
function toNominationRow(api: any): SellerNomination & { region?: string } {
  const raw = String(api.status ?? '').toUpperCase();
  const status: NominationStatus =
    raw === 'APPROVED'
      ? 'approved'
      : raw === 'REJECTED' || raw === 'WITHDRAWN'
        ? 'rejected'
        : raw === 'LIVE'
          ? 'live'
          : 'pending';
  return {
    id: String(api.id),
    product: String(api.product?.name ?? api.productId ?? ''),
    seller: String(api.seller?.businessName ?? api.sellerId ?? ''),
    sellerRating: Number(api.seller?.sellerRating ?? 0) || 0,
    category: String(api.product?.category?.name ?? ''),
    originalPrice: Number(api.product?.mrp ?? 0) || 0,
    proposedDiscount: `${Number(api.proposedDiscountPercent ?? 0) || 0}%`,
    stockAllocated: Number(api.stockAllocated ?? 0) || 0,
    note: String(api.sellerNote ?? ''),
    submittedAt: fmtDate(api.submittedAt),
    status,
    region: api.deal?.regionCode ?? undefined,
  };
}
...
export default function FlashDealsPage() {
  const [activeTab, setActiveTab] = useState<'deals' | 'create' | 'nominations' | 'analytics'>(
    'deals',
  );
  // Empty until the API answers: the eight fixture campaigns this started
  // with were reported as real whenever a market had none.
  const [deals, setDeals] = useState<FlashDeal[]>([]);
  const [nominations, setNominations] = useState<SellerNomination[]>([]);
  ...
  const {
    ...
    toast,
    showToast,
  } = useAdminData(() => adminMarketplaceApi.getFlashDeals({ country }), [country]);
  const { data: nomApiData } = useAdminData(
    () => adminMarketplaceApi.getNominations({ country }),
    [country],
  );
  ...
  useEffect(() => {
    // Rows carry `regionCode`; the client-side region filter reads `region`.
    if (Array.isArray(apiData?.data)) {
      setDeals(apiData.data.map(toDealRow));
    }
  }, [apiData]);

  useEffect(() => {
    if (Array.isArray(nomApiData?.data)) {
      setNominations(nomApiData.data.map(toNominationRow));
    }
  }, [nomApiData]);

  // Region filtering
  const {
    filtered: regionFilteredDeals,
    regionLabel,
    isFiltered: isRegionFiltered,
    formatCurrencyValue,
  } = useMarketplaceRegionFilter(deals);
  ...
  const TABS = [
    {
      ...
      icon: Zap,
      count: regionFilteredDeals.filter((d) => ['active', 'scheduled'].includes(d.status)).length,
    },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {isRegionFiltered ? `${regionLabel} — ` : ''}Create, schedule, monitor flash deals and
            approve seller nominations
          </p>
        </div>
        <button
          onClick={() => setActiveTab('create')}
          className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm transition-colors"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          {
            label: 'Total Revenue',
            value: formatCurrencyValue(totalRevenue, { compact: true }),
            icon: DollarSign,
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 text-sm border border-slate-200 rounded-lg bg-white outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-300"
              />
            </div>
            <select
              value={statusFilter}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {regionFilteredDeals
                  ...
                  .map((deal) => {
                    const pct =
                      deal.stockLimit > 0 ? Math.round((deal.sold / deal.stockLimit) * 100) : 0;
                    ...
                    return (
                      <tr key={deal.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-4 text-center">
                          <span className="text-xs font-bold bg-red-50 text-red-700 px-2 py-1 rounded-lg">
                            {deal.discount}
                          </span>
                        </td>
                        <td className="px-4 py-4 min-w-[160px]">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-slate-600 whitespace-nowrap">
                              {deal.sold}/{deal.stockLimit}
                            </span>
                          </div>
                          <p className="text-[10px] text-slate-400 mt-0.5">{pct}% sold</p>
                        </td>
                        <td className="px-4 py-4 text-right font-bold text-slate-900">
                          {formatCurrencyValue(deal.revenue, { compact: true })}
                        </td>
                        ...
                        <td className="px-4 py-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            {deal.status === 'scheduled' && (
                              <button
                                onClick={() => setDeals((d) => d.filter((x) => x.id !== deal.id))}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="bg-linear-to-r from-red-600 to-rose-600 px-6 py-4">
              <h2 className="text-lg font-black text-white flex items-center gap-2">
                <Zap className="w-5 h-5 fill-white" /> Create Flash Deal
              </h2>
              <p className="text-sm text-red-100 mt-0.5">
                Set up a new time-limited flash deal for the marketplace
              </p>
            </div>
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label
                    className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 block"
                    htmlFor="deal-name"
                  >
                    Deal Name *
                  </label>
                  <input
                    id="deal-name"
                    placeholder="e.g. Smartphone Mega Sale"
                    className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-300"
                  />
                </div>
                <div>
                  <label
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="text-[10px] text-slate-400 mb-1 block" htmlFor="type">
                      Type
                    </label>
                    <select
                      id="type"
                  ...
                  <div>
                    <label className="text-[10px] text-slate-400 mb-1 block" htmlFor="value">
                      Value
                    </label>
                    <input
                      id="value"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] text-slate-400 mb-1 block" htmlFor="source">
                      Source
                    </label>
                    <select
                      id="source"
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label
                      className="text-[10px] text-slate-400 mb-1 block"
                      htmlFor="total-stock-limit"
                    >
                      Total Stock Limit
                    </label>
                    <input
                      id="total-stock-limit"
                      type="number"
                      placeholder="e.g. 500"
                      className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none"
                    />
              ...
              <div>
                ...
                <div className="flex gap-3 flex-wrap">
                  {[
                    ...
                    <label
                      key={p}
                      className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 cursor-pointer hover:border-red-300 transition-colors"
                    >
                      <input
                        type="checkbox"
                        defaultChecked={p === 'Homepage Flash Deals'}
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-200">
                <button
                  ...
                  className="flex-1 bg-slate-100 text-slate-700 font-bold py-3 rounded-xl text-sm hover:bg-slate-200 transition-colors"
                >
                  Cancel
                </button>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-amber-800">
                  {pendingNoms} nomination{pendingNoms > 1 ? 's' : ''} pending review
                </p>
                <p className="text-xs text-amber-600">
                  Sellers have nominated products for upcoming flash deals. Review and approve to
                  include them.
                </p>
              </div>
            </div>
                  ...
                  return (
                    <tr key={nom.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-5 py-4">
                        ...
                        {nom.note && (
                          <p className="text-[10px] text-slate-400 mt-1 italic">"{nom.note}"</p>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-4 text-center">
                        <span
                          className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${ns.bg} ${ns.text}`}
                        >
                          {ns.label}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          {nom.status === 'pending' && (
                            <>
                                  ...
                                  execute(
                                    ...
                                    () =>
                                      setNominations((n) =>
                                        n.map((x) =>
                                          x.id === nom.id
                                            ? { ...x, status: 'approved' as NominationStatus }
                                            : x,
                                        ),
                                      ),
                                  )
                              ...
                              <button
                                onClick={() => setShowRejectModal(nom.id)}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-blue-500" /> Hourly Sales Today
            </h3>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-200">
              <h3 className="font-bold text-slate-900 flex items-center gap-2">
                <Flame className="w-5 h-5 text-red-500" /> Top Performing Deals
              </h3>
            </div>
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200 text-xs text-slate-500">
                ...
                {deals
                  ...
                  .map((deal, i) => {
                    ...
                    const conversionRate =
                      deal.views > 0 ? ((deal.orders / deal.views) * 100).toFixed(1) : '0';
                    return (
                      <tr key={deal.id} className="hover:bg-slate-50/50">
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            <span
                              className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black ${i === 0 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}
                            >
                              {i + 1}
                            </span>
                            <div>
                        ...
                        <td className="px-4 py-3.5 text-right font-black text-emerald-700">
                          ₹{(deal.revenue / 100000).toFixed(1)}L
                        </td>
                        <td className="px-4 py-3.5 text-center font-bold text-slate-700">
                          {deal.orders}
                        </td>
                        <td className="px-4 py-3.5 text-center text-slate-600">
                          {(deal.views / 1000).toFixed(1)}K
                        </td>
                        <td className="px-4 py-3.5 min-w-[120px]">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-slate-100 rounded-full h-1.5">
                              <div
                                className={`h-full rounded-full ${pct >= 90 ? 'bg-red-500' : 'bg-blue-500'} w-[--bar-w]`}
                                /* @ts-ignore */ {...{
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
            ...
            <p className="text-sm text-slate-500 mb-4">
              Select a reason for rejecting this product nomination.
            </p>
            <select
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm mb-3 focus:outline-none"
            ...
            <textarea
              ...
              rows={3}
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm resize-none focus:outline-none mb-4"
            />
            <div className="flex gap-3">
              <button
              ...
              <button
                onClick={() => {
                  execute(
                    () =>
                      adminMarketplaceApi.rejectNomination(
                        showRejectModal!,
                        'Does not meet flash deal criteria',
                      ),
                    'Nomination rejected',
                    () =>
                      setNominations((n) =>
                        n.map((x) =>
                          x.id === showRejectModal
                            ? { ...x, status: 'rejected' as NominationStatus }
                            : x,
                        ),
                      ),
                  );
