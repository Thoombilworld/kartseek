...
const CARDS: GiftCard[] = [
  ...
  {
    id: 'GC-002',
    code: 'KART-GIFT-D4E5F6',
    denomination: 10000,
    balance: 3200,
    country: 'India',
    status: 'Partially Used',
    purchasedBy: 'Amit Patel',
    recipientEmail: 'sneha@email.com',
    purchaseDate: '2026-05-15',
    expiryDate: '2027-05-15',
    usageHistory: [
      { orderId: 'ORD-8820', amount: 4500, date: '2026-05-20' },
      { orderId: 'ORD-8845', amount: 2300, date: '2026-06-02' },
    ],
    theme: 'Wedding',
  ...
  {
    id: 'GC-003',
    code: 'KART-GIFT-G7H8I9',
    denomination: 2000,
    balance: 0,
    country: 'India',
    status: 'Redeemed',
    purchasedBy: 'Sneha Nair',
    recipientEmail: 'amit@email.com',
    purchaseDate: '2026-04-01',
    expiryDate: '2027-04-01',
    usageHistory: [{ orderId: 'ORD-8750', amount: 2000, date: '2026-04-10' }],
    theme: 'Thank You',
    message: 'Thanks for being awesome!',
  },
  {
    id: 'GC-004',
    code: 'KART-GIFT-J1K2L3',
    denomination: 25000,
    balance: 25000,
    country: 'UAE',
    status: 'Active',
    purchasedBy: 'Ahmed Al-Farsi',
    recipientEmail: 'khalid@email.com',
  ...
  {
    id: 'GC-005',
    code: 'KART-GIFT-M4N5O6',
    denomination: 1000,
    balance: 1000,
    country: 'India',
    status: 'Expired',
    purchasedBy: 'System Promo',
    recipientEmail: 'winner@email.com',
    purchaseDate: '2025-06-01',
    expiryDate: '2026-06-01',
    usageHistory: [],
    theme: 'Promotional',
    message: 'Contest winner reward',
  },
  {
    id: 'GC-006',
    code: 'KART-GIFT-P7Q8R9',
    denomination: 3000,
    balance: 3000,
    country: 'India',
    status: 'Disabled',
    purchasedBy: 'Fraud Review',
    recipientEmail: 'suspect@email.com',
...
const STATUS_STYLES: Record<string, string> = {
  ...
  'Partially Used': 'bg-blue-50 text-blue-700',
...
const THEME_COLORS: Record<string, string> = {
  ...
  'Thank You': 'from-blue-500 to-indigo-600',
  Eid: 'from-emerald-500 to-teal-600',
  Promotional: 'from-slate-600 to-slate-800',
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl overflow-y-auto animate-slide-left">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <p className="text-xs text-slate-500">{c.purchaseDate}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5">
          <div
            className={`bg-gradient-to-br ${THEME_COLORS[c.theme] || THEME_COLORS.General} rounded-2xl p-6 text-white relative overflow-hidden`}
          >
            ...
            <p className="font-mono text-sm mt-3 bg-white/20 inline-block px-3 py-1 rounded-lg">
              {c.code}
            </p>
            {c.message && <p className="text-xs opacity-80 mt-3 italic">"{c.message}"</p>}
            <p className="text-[10px] opacity-60 mt-2">
              {c.theme} · Expires {c.expiryDate}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(c.balance / c.denomination) * 100}%` }}
              />
            </div>
            <div className="flex justify-between mt-1">
              <span className="text-xs font-bold text-emerald-600">{fmt(c.balance)} remaining</span>
              <span className="text-xs text-slate-400">{usedPct.toFixed(0)}% used</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Purchased By</p>
              <p className="text-sm font-bold text-slate-900">{c.purchasedBy}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Recipient</p>
              <p className="text-sm font-bold text-slate-900">{c.recipientEmail}</p>
            </div>
          </div>

          {c.usageHistory.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-slate-900 mb-2">Usage History</h3>
              {c.usageHistory.map((u, i) => (
                ...
                >
                  <div>
                    <p className="text-xs font-bold text-blue-700 font-mono">{u.orderId}</p>
          ...
          <div className="flex gap-3">
            {c.status === 'Active' && (
              <button className="flex-1 bg-red-50 hover:bg-red-100 text-red-700 py-3 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2">
                <Ban className="w-4 h-4" /> Disable Card
              </button>
            )}
            {c.status === 'Disabled' && (
              <button className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2">
                <CheckCircle className="w-4 h-4" /> Re-Enable
              </button>
            )}
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="denomination">
                Denomination (₹)
              </label>
              <select
                id="denomination"
                value={form.denomination}
                onChange={(e) => u('denomination', +e.target.value)}
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="quantity">
                Quantity
              </label>
              <input
                id="quantity"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => u('recipientEmail', e.target.value)}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="theme">
                Theme
              </label>
              <select
                id="theme"
                value={form.theme}
                onChange={(e) => u('theme', e.target.value)}
                className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none"
              >
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 mb-1 block" htmlFor="validity">
                Validity
              </label>
              <select
                id="validity"
                value={form.expiryMonths}
                onChange={(e) => u('expiryMonths', +e.target.value)}
                className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none"
              >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-slate-200 rounded-xl p-3 text-sm resize-none h-20 outline-none"
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            onClick={() => onIssue(form)}
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
          >
            <Gift className="w-4 h-4" /> Issue{' '}
            {form.quantity > 1 ? `${form.quantity} Cards` : 'Card'}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Gift Cards</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            Issue, track, and manage gift card inventory
          </p>
        </div>
        <div className="flex gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-gradient-to-br from-pink-500 to-purple-600 rounded-xl p-5 text-white">
          <p className="text-sm font-bold opacity-80">Total Issued</p>
          <p className="text-2xl font-black mt-1">{fmt(totalIssued)}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-slate-500">Outstanding Balance</p>
          <p className="text-xl font-black text-emerald-600">{fmt(totalBalance)}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-slate-500">Active Cards</p>
          <p className="text-xl font-black text-blue-600">
            {
              regionFiltered.filter((c) => c.status === 'Active' || c.status === 'Partially Used')
                .length
            }
          </p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          ...
          <p className="text-xl font-black text-slate-600">
            {regionFiltered.filter((c) => c.status === 'Redeemed').length}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search code, buyer, or recipient..."
        ...
        <div className="flex gap-2 flex-wrap">
          {['all', 'Active', 'Partially Used', 'Redeemed', 'Expired', 'Disabled'].map((s) => (
            <button
              key={s}
              onClick={() => {
                setFilter(s);
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              <th className="px-4 py-3 text-left font-semibold text-slate-500 text-xs">Card</th>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 text-xs">Theme</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 text-xs">Expiry</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {paged.length === 0 ? (
              <tr>
                <td colSpan={8}>
                  <MarketplaceEmptyState title="No gift cards found" icon={Gift} />
                </td>
              </tr>
            ) : (
              paged.map((c) => (
                ...
                >
                  <td className="px-4 py-3.5">
                    <p className="font-mono font-bold text-blue-700 text-xs">{c.code}</p>
                    <p className="text-[10px] text-slate-400">
                      {c.id} · {c.purchaseDate}
                    </p>
                  </td>
                  <td className="px-4 py-3.5">
                    <p className="text-xs font-bold text-slate-900">{c.recipientEmail}</p>
                    <p className="text-[10px] text-slate-400">by {c.purchasedBy}</p>
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <span
                      className={`font-black ${c.balance === 0 ? 'text-slate-400' : 'text-emerald-600'}`}
                    >
                      {fmt(c.balance)}
                    </span>
                  </td>
      ...
      {showIssue && (
        <IssueCardModal
          onIssue={(data) => {
            execute(
              () => adminMarketplaceApi.sendNotification(data),
              `${data.quantity} gift card(s) issued`,
              () => refetch(),
            );
            setShowIssue(false);
          }}
