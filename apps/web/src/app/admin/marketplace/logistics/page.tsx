...
const SHIPMENTS: Shipment[] = [
  {
    ...
    date: '2026-06-06',
    timeline: [
      { date: '2026-06-06 10:00', event: 'Picked up from seller', location: 'Mumbai Hub' },
      { date: '2026-06-06 14:00', event: 'Reached sort facility', location: 'Mumbai Airport' },
      { date: '2026-06-06 22:00', event: 'In transit', location: 'En route to Bangalore' },
    ],
  },
  {
    id: 'SHP-002',
    ...
    date: '2026-06-03',
    timeline: [
      { date: '2026-06-03', event: 'Picked up', location: 'Delhi Hub' },
      { date: '2026-06-04', event: 'In transit', location: 'Hyderabad' },
      {
        date: '2026-06-05',
        event: 'Delivery attempted — Customer not available',
        location: 'Chennai',
      },
      { date: '2026-06-05', event: 'Delivery failed — 3 attempts exhausted', location: 'Chennai' },
    ],
  },
  {
    id: 'SHP-003',
    ...
    date: '2026-06-04',
    timeline: [
      { date: '2026-06-04', event: 'Shipped', location: 'Bangalore' },
      { date: '2026-06-05', event: 'In transit', location: 'Pune Hub' },
      { date: '2026-06-06 08:00', event: 'Out for delivery', location: 'Pune' },
    ],
  },
  {
    id: 'SHP-004',
    ...
    country: 'UAE',
    carrier: 'Aramex',
    trackingNumber: 'ARX1122334455',
    status: 'Delivered',
    estimatedDelivery: '2026-06-05',
    actualDelivery: '2026-06-04',
    origin: 'Dubai',
    ...
    date: '2026-06-02',
    timeline: [
      { date: '2026-06-02', event: 'Shipped', location: 'Dubai Hub' },
      { date: '2026-06-03', event: 'In transit', location: 'Al Ain' },
      { date: '2026-06-04', event: 'Delivered', location: 'Abu Dhabi' },
    ],
  },
  {
    id: 'SHP-005',
    ...
    weight: '2.0 kg',
    date: '2026-06-06',
    timeline: [{ date: '2026-06-06', event: 'Shipped — label created', location: 'Mumbai Hub' }],
  },
  {
    id: 'SHP-006',
    orderId: 'ORD-8870',
    ...
    date: '2026-05-30',
    timeline: [
      { date: '2026-05-30', event: 'Shipped', location: 'Varanasi' },
      { date: '2026-06-01', event: 'Customer refused delivery', location: 'Kochi' },
      { date: '2026-06-02', event: 'RTO initiated', location: 'Kochi' },
    ],
  },
  {
    id: 'SHP-007',
    ...
    date: '2026-06-06',
    timeline: [
      { date: '2026-06-06', event: 'Order received — awaiting pickup', location: 'Riyadh' },
    ],
  },
];
...
const CARRIER_COLORS: Record<string, string> = {
  Delhivery: 'bg-red-50 text-red-700',
  BlueDart: 'bg-blue-50 text-blue-700',
  Aramex: 'bg-orange-50 text-orange-700',
  FedEx: 'bg-purple-50 text-purple-700',
  DTDC: 'bg-yellow-50 text-yellow-700',
  SMSA: 'bg-emerald-50 text-emerald-700',
};
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-left">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-lg font-black text-slate-900">{s.id}</h2>
            <p className="text-xs text-slate-500">{s.orderId}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5">
          <div className="flex gap-2 flex-wrap">
            <span
              className={`text-[10px] font-bold px-2.5 py-1 rounded-md ${STATUS_STYLES[s.status]}`}
            >
              {s.status}
            </span>
            <span
              className={`text-[10px] font-bold px-2.5 py-1 rounded-md ${CARRIER_COLORS[s.carrier] || 'bg-slate-100 text-slate-600'}`}
            >
              {s.carrier}
            </span>
            {s.slaBreached && (
          ...
          <div className="bg-slate-50 rounded-xl p-4">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Tracking: <span className="font-mono text-blue-600">{s.trackingNumber}</span>
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-emerald-50 rounded-xl p-4 flex items-center gap-4">
            <div className="text-center">
              ...
              <p className="text-xs font-bold mt-1">{s.origin}</p>
            ...
            <div className="text-center">
              ...
              <p className="text-xs font-bold mt-1">{s.destination}</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <p className="text-sm font-bold text-slate-900">{s.customer}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Seller</p>
              <p className="text-sm font-bold text-slate-900">{s.seller}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Est. Delivery</p>
              <p className="text-sm font-bold text-slate-900">{s.estimatedDelivery}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Weight</p>
              <p className="text-sm font-bold text-slate-900">{s.weight}</p>
            </div>
          </div>
            ...
            <div className="space-y-0 relative">
              ...
              {s.timeline.map((t, i) => (
                <div key={i} className="flex items-start gap-3 py-2 relative">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 z-10 ${i === s.timeline.length - 1 ? (s.status === 'Delivered' ? 'bg-emerald-600' : s.status === 'Failed' || s.status === 'RTO' ? 'bg-red-600' : 'bg-blue-600') : 'bg-slate-200'}`}
                  >
                    {i === s.timeline.length - 1 ? (
                      s.status === 'Delivered' ? (
                        <CheckCircle className="w-3 h-3 text-white" />
                      ) : s.status === 'Failed' || s.status === 'RTO' ? (
                  ...
                  <div>
                    <p className="text-xs font-bold text-slate-900">{t.event}</p>
...
export default function LogisticsPage() {
  ...
  const {
    ...
    regionLabel,
    isFiltered,
  } = useMarketplaceRegionFilter(SHIPMENTS);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Shipping & Logistics</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            {isFiltered ? `${regionLabel} — ` : ''}Track shipments, monitor carriers, and resolve
            delivery issues
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        {[
          {
            l: 'In Transit',
            v: regionFiltered.filter((s) => s.status === 'In Transit').length,
            c: 'text-indigo-600',
          },
          {
            l: 'Out for Delivery',
            v: regionFiltered.filter((s) => s.status === 'Out for Delivery').length,
            c: 'text-amber-600',
          },
          {
            l: 'Delivered',
            v: regionFiltered.filter((s) => s.status === 'Delivered').length,
            c: 'text-emerald-600',
          },
          {
            l: 'Failed',
            v: regionFiltered.filter((s) => s.status === 'Failed').length,
            c: 'text-red-600',
          },
          {
            l: 'RTO',
            v: regionFiltered.filter((s) => s.status === 'RTO').length,
            c: 'text-red-700',
          },
          {
            l: 'SLA Breached',
            v: slaBreachedCount,
            c: slaBreachedCount > 0 ? 'text-red-600' : 'text-emerald-600',
          },
        ].map((k) => (
          <div key={k.l} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <p className={`text-xl font-black ${k.c}`}>{k.v}</p>
            <p className="text-[10px] text-slate-500 mt-1">{k.l}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search product, tracking number, or customer..."
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {paged.length === 0 ? (
              <tr>
                <td colSpan={7}>
                  <MarketplaceEmptyState title="No shipments found" icon={Truck} />
                </td>
              </tr>
            ) : (
              paged.map((s) => (
                <tr
                  key={s.id}
                  className={`hover:bg-slate-50/50 cursor-pointer transition-colors ${s.slaBreached ? 'bg-red-50/30' : ''}`}
                  onClick={() => setSelected(s)}
                  tabIndex={0}
                  onKeyDown={activateOnKey(() => setSelected(s))}
                >
                  <td className="px-4 py-3.5">
                    <p className="font-bold text-slate-900 text-xs">{s.product}</p>
                    <p className="text-[10px] text-slate-400">
                      {s.id} · {s.orderId} ·{' '}
                      <CountryFlag code={COUNTRY_TO_CODE[s.country] || 'IN'} size="sm" />
                    </p>
                  </td>
                  <td className="px-4 py-3.5">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${CARRIER_COLORS[s.carrier] || 'bg-slate-100 text-slate-600'}`}
                    >
                      {s.carrier}
                    </span>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2">
          <p className="text-xs text-slate-500">{filtered.length} shipments</p>
