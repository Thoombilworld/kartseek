...
import {
  ...
  GripVertical, Edit, Trash2, Image, Package, Tag, LayoutGrid, Megaphone,
...
const DEFAULT_HOMEPAGE_SECTIONS = [
  { id: 'sec-hero', type: 'hero_slider', title: 'Hero Banner Slider', visible: true, banners: [
    { id: 'b1', tag: 'MEGA SALE', headline: 'Up to 70% Off\nOn Top Brands', subheadline: 'Electronics, Fashion, Home & more', cta: 'Shop Now', ctaHref: '/marketplace', gradient: 'from-blue-600 to-indigo-700' },
    { id: 'b2', tag: 'FASHION FEST', headline: 'New Season Styles\nMin 30% Off', subheadline: 'Latest trends from top brands', cta: 'Explore Fashion', ctaHref: '/marketplace/category/fashion', gradient: 'from-rose-600 to-pink-600' },
    { id: 'b3', tag: 'TECH DEALS', headline: 'Gadget Fest\nFlat 40% Off', subheadline: 'Smartphones, Laptops, Accessories', cta: 'Shop Electronics', ctaHref: '/marketplace/category/electronics', gradient: 'from-violet-600 to-purple-700' },
  ]},
  ...
  { id: 'sec-appliances', type: 'product_section', title: 'Appliances', subtitle: 'Smart home essentials', categoryKey: 'appliances', viewAllHref: '/marketplace/category/appliances', iconName: 'Tv', showBrandCards: true, brandCategory: 'appliances', visible: true },
  { id: 'sec-trending', type: 'product_section', title: 'Trending Now', subtitle: 'What everyone is buying', categoryKey: 'trending', viewAllHref: '/marketplace/trending', iconName: 'TrendingUp', borderAccent: 'bg-linear-to-r from-violet-500 via-purple-500 to-fuchsia-500', visible: true },
  { id: 'sec-new', type: 'product_section', title: 'New Arrivals', subtitle: 'Just landed on KARTSEEK', categoryKey: 'new-arrivals', viewAllHref: '/marketplace/new-arrivals', iconName: 'Sparkles', borderAccent: 'bg-linear-to-r from-blue-500 via-cyan-500 to-teal-500', visible: true },
  { id: 'sec-best', type: 'product_section', title: 'Best Sellers', subtitle: 'Top-rated by customers', categoryKey: 'best-sellers', viewAllHref: '/marketplace/best-sellers', iconName: 'Award', borderAccent: 'bg-linear-to-r from-amber-500 via-yellow-500 to-orange-500', visible: true },
  { id: 'sec-deals', type: 'product_section', title: 'Deals of the Day', subtitle: 'Massive savings, limited time', categoryKey: 'deals', viewAllHref: '/marketplace/deals', iconName: 'Flame', borderAccent: 'bg-linear-to-r from-red-500 via-rose-500 to-pink-500', visible: true },
  { id: 'sec-recommended', type: 'product_section', title: 'Recommended For You', subtitle: 'Personalized picks', categoryKey: 'recommended', viewAllHref: '/marketplace/recommended', iconName: 'Heart', visible: true },
  { id: 'sec-sponsored', type: 'sponsored_products', title: 'Sponsored Products', subtitle: 'Featured by sellers', visible: true },
  { id: 'sec-country', type: 'country_banners', title: 'Shop by Country', subtitle: 'Localized experience, local delivery', visible: true },
  { id: 'sec-all-cat', type: 'category_grid', title: 'Explore All Categories', subtitle: 'Browse everything', viewAllHref: '/marketplace/category-list', visible: true },
  { id: 'sec-faq', type: 'faq', title: 'Frequently Asked Questions', visible: true, items: [
...
function SortableRow({ id, section, onEdit, onDelete, onToggle }: {
  ...
  const getIcon = () => {
    switch (section.type) {
      case 'hero_slider': return <Image className="w-4 h-4 text-indigo-500" />;
      case 'trust_badges': return <ShieldCheck className="w-4 h-4 text-emerald-500" />;
      case 'category_grid': return <LayoutGrid className="w-4 h-4 text-teal-500" />;
      case 'flash_deals': return <Zap className="w-4 h-4 text-red-500" />;
      case 'product_section': return <Package className="w-4 h-4 text-blue-500" />;
      case 'campaign_banner': return <Megaphone className="w-4 h-4 text-rose-500" />;
      case 'brand_promo_row': return <Award className="w-4 h-4 text-purple-500" />;
      case 'country_banners': return <MapPin className="w-4 h-4 text-cyan-500" />;
      case 'sponsored_products': return <Star className="w-4 h-4 text-amber-500" />;
...
export default function MarketplacePageBuilder() {
  ...
  const { data: apiData, loading, error, refetch, toast, showToast } = useAdminData(
    () => adminMarketplaceApi.getPageLayout(),
    []
  );
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Design and manage all marketplace module pages with full CMS control</p>
        </div>
        <div className="flex items-center gap-3">
          {message && (
            <span className={`text-sm font-medium flex items-center gap-1 ${message.includes('Error') ? 'text-rose-500' : 'text-emerald-600'}`}>
      ...
      <div className="flex gap-5 items-start">
        ...
        <div className="w-56 shrink-0 space-y-5">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg text-xs font-medium transition-colors flex items-center gap-2 ${
                      selectedPage === page.id ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'
                    }`}
                  >
          ...
          {/* Seller Sync Status */}
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 block">Seller Sync</label>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-xs text-emerald-700 font-medium">Connected</span>
            </div>
            <p className="text-[10px] text-slate-400 mt-1">Seller products auto-inject into matching category sections</p>
          </div>
        </div>
          ...
          <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 min-h-[700px] relative">
            ...
            {sections.length === 0 && !isLoading ? (
              <div className="text-center py-16 bg-white rounded-xl border border-dashed border-slate-300">
                ...
                <button onClick={() => setShowTypeMenu(true)} className="mt-4 text-blue-600 font-bold text-sm hover:underline">Create First Section</button>
              </div>
            ) : (
            ...
            {/* Add Section CTA */}
            <button onClick={() => setShowTypeMenu(true)} className="w-full mt-3 border-2 border-dashed border-slate-300 rounded-xl py-4 flex flex-col items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-400 hover:bg-blue-50/30 transition-colors">
