...
const PAYOUTS: Payout[] = [
  {
    ...
    payoutCycle: 'Monthly',
    bankAccount: 'HDFC ****4291',
    settlementReport: [
      { orderId: 'ORD-8890', amount: 134900, commission: 4047, net: 130853, date: '2026-06-06' },
      { orderId: 'ORD-8875', amount: 114900, commission: 3447, net: 111453, date: '2026-06-05' },
      { orderId: 'ORD-8860', amount: 159800, commission: 4794, net: 155006, date: '2026-06-04' },
    ],
  },
  {
    id: 'PAY-6002',
    ...
    nextPayout: '2026-06-15',
    payoutCycle: 'Monthly',
    bankAccount: 'ICICI ****7823',
    settlementReport: [
      { orderId: 'ORD-8885', amount: 79999, commission: 3999, net: 76000, date: '2026-06-03' },
      { orderId: 'ORD-8880', amount: 169999, commission: 8499, net: 161500, date: '2026-06-02' },
    ],
  },
  {
    id: 'PAY-6003',
    ...
    bankAccount: 'SBI ****1456',
    settlementReport: [
      { orderId: 'ORD-8882', amount: 16995, commission: 2379, net: 14616, date: '2026-06-04' },
    ],
  },
  {
    id: 'PAY-6004',
    ...
    payoutCycle: 'Monthly',
    bankAccount: 'Emirates NBD ****9012',
    settlementReport: [
      { orderId: 'ORD-8878', amount: 52000, commission: 3120, net: 48880, date: '2026-06-02' },
    ],
  },
  {
    id: 'PAY-6005',
    ...
    tds: 1800,
    payoutStatus: 'hold',
    lastPayout: '2026-04-30',
    nextPayout: 'On Hold',
    payoutCycle: 'Weekly',
    bankAccount: 'Axis ****5678',
    settlementReport: [],
  },
  {
    id: 'PAY-6006',
    ...
    nextPayout: '2026-06-14',
    payoutCycle: 'Bi-Weekly',
    bankAccount: 'BOI ****3344',
    settlementReport: [
      { orderId: 'ORD-8870', amount: 8999, commission: 1259, net: 7740, date: '2026-05-30' },
    ],
  },
];
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-left">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-lg font-black text-slate-900">{p.seller}</h2>
            <p className="text-xs text-slate-500">
              {p.id} · <CountryFlag code={COUNTRY_TO_CODE[p.country] || 'IN'} size="sm" />{' '}
              {p.country}
            </p>
          </div>
          <button
        ...
        <div className="p-6 space-y-5">
          <div className="flex gap-2">
            <span
              className={`text-[10px] font-bold px-2.5 py-1 rounded-md capitalize ${STATUS_STYLES[p.payoutStatus]}`}
            >
              {p.payoutStatus}
            </span>
          ...
          <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-xl p-5 text-white text-center">
            <p className="text-sm font-bold opacity-80">Pending Payout</p>
            <p className="text-3xl font-black mt-1">{formatCurrency(p.pendingAmount)}</p>
            <p className="text-xs opacity-60 mt-1">Next: {p.nextPayout}</p>
          </div>
          ...
          <div>
            ...
            <div className="bg-slate-50 rounded-xl p-4 space-y-2">
              <div className="flex justify-between">
                <span className="text-xs text-slate-600">Gross Amount</span>
                <span className="text-xs font-bold text-slate-900">
                  {formatCurrency(p.pendingAmount)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-red-500">− Commission</span>
                <span className="text-xs font-bold text-red-500">
                  {formatCurrency(p.commission)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-red-500">− Refund Adjustments</span>
                <span className="text-xs font-bold text-red-500">
                  {formatCurrency(p.refundAdj)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-red-500">− TDS</span>
                <span className="text-xs font-bold text-red-500">{formatCurrency(p.tds)}</span>
              </div>
              <div className="border-t border-slate-200 pt-2 flex justify-between">
                <span className="text-sm font-bold text-slate-900">Net Payout</span>
                <span className="text-sm font-black text-emerald-700">
                  {formatCurrency(netPayout > 0 ? netPayout : 0)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Total Paid (Lifetime)</p>
              <p className="text-sm font-bold text-slate-900">{formatCurrency(p.totalPaid)}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Last Payout</p>
              <p className="text-sm font-bold text-slate-900">{p.lastPayout}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Bank Account</p>
              <p className="text-sm font-bold text-slate-900">{p.bankAccount}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] text-slate-500">Cycle</p>
              <p className="text-sm font-bold text-slate-900">{p.payoutCycle}</p>
            </div>
          </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="px-3 py-2 text-left text-slate-500">Order</th>
                      <th className="px-3 py-2 text-right text-slate-500">Amount</th>
                      <th className="px-3 py-2 text-right text-slate-500">Commission</th>
                      <th className="px-3 py-2 text-right text-slate-500">Net</th>
                      <th className="px-3 py-2 text-left text-slate-500">Date</th>
                    </tr>
                  </thead>
                  <tbody>
                    {p.settlementReport.map((s, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0">
                        <td className="px-3 py-2 font-mono font-bold text-blue-700">{s.orderId}</td>
          ...
          <div className="space-y-3">
            ...
            {p.payoutStatus !== 'hold' && p.payoutStatus !== 'settled' && (
              <button
                onClick={onHold}
              ...
              >
                <Lock className="w-4 h-4" /> Put On Hold
              </button>
            )}
            {p.payoutStatus === 'hold' && (
              <button
...
export default function PayoutsPage() {
  ...
  const {
    ...
    toast,
    showToast,
  } = useAdminData(() => adminMarketplaceApi.getPayouts({ country }), [country]);
  ...
  const {
    ...
    isFiltered,
    formatCurrencyValue,
  } = useMarketplaceRegionFilter(PAYOUTS);
  ...
  const filtered = regionFiltered.filter((p) => {
    ...
    if (search && !p.seller.toLowerCase().includes(search.toLowerCase()) && !p.id.includes(search))
      return false;
    return true;
  });
  ...
  const handleRelease = (p: Payout) => {
    execute(
      () => adminMarketplaceApi.processPayout(p.id, { action: 'release' }),
      `Payout released for ${p.seller}`,
      () => refetch(),
    );
    setSelected(null);
  };
  const handleHold = (p: Payout) => {
    execute(
      () => adminMarketplaceApi.processPayout(p.id, { action: 'hold' }),
      `Payout held for ${p.seller}`,
      () => refetch(),
    );
    setSelected(null);
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Seller Payouts</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            {isFiltered ? `${regionLabel} — ` : ''}Settlement reports, payout release & hold
            management
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl p-5 text-white">
          <p className="text-sm font-bold opacity-80">Pending Payouts</p>
          <p className="text-2xl font-black mt-1">{fmt(totalPending)}</p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          ...
          <p className="text-xl font-black text-blue-600">
            {regionFiltered.filter((p) => p.payoutStatus === 'processing').length}
          </p>
        </div>
        <div className="bg-white border border-red-200 rounded-xl p-4 shadow-sm">
          <p className="text-xs text-red-600 font-bold">On Hold</p>
          <p className="text-xl font-black text-red-600">
            {regionFiltered.filter((p) => p.payoutStatus === 'hold').length}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search seller or payout ID..."
        ...
        <div className="flex gap-2">
          {['all', 'pending', 'processing', 'settled', 'hold'].map((s) => (
            <button
              key={s}
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8}>
                  <MarketplaceEmptyState title="No payouts found" icon={Wallet} />
                </td>
              </tr>
            ) : (
              filtered.map((p) => (
                <tr
                  key={p.id}
                  className={`hover:bg-slate-50/50 cursor-pointer transition-colors ${p.payoutStatus === 'hold' ? 'bg-red-50/30' : ''}`}
                  onClick={() => setSelected(p)}
                  tabIndex={0}
                  onKeyDown={activateOnKey(() => setSelected(p))}
                >
                  <td className="px-4 py-3.5">
                    <p className="font-bold text-slate-900 text-xs">{p.seller}</p>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md capitalize ${STATUS_STYLES[p.payoutStatus]}`}
                    >
                      {p.payoutStatus}
                    </span>
                  </td>
