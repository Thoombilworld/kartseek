...
type ModalState = {
  productId: string;
  productName: string;
  action: 'approve' | 'reject' | 'correction' | 'suspend';
} | null;
...
export default function AllProductsPage() {
  ...
  const {
    ...
    formatCurrencyValue,
  } = useMarketplaceRegionFilter(products);
    ...
    setProducts((prev) =>
      prev.map((p) => {
        ...
        if (modal.action === 'approve')
          return { ...p, approval: 'approved', visibility: 'published' };
        if (modal.action === 'reject')
          return { ...p, approval: 'rejected', visibility: 'unpublished' };
  ...
  const toggleVisibility = (id: string) => {
    setProducts((prev) =>
      prev.map((p) =>
        p.id === id
          ? { ...p, visibility: p.visibility === 'published' ? 'unpublished' : 'published' }
          : p,
      ),
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {isRegionFiltered ? `${regionLabel} — ` : ''}Approve, publish, feature, or suspend
            marketplace products. Only approved + published products are visible to customers.
          </p>
        </div>
        <div className="flex gap-2">
          <Link
            href={AdminMarketplaceRoutes.productApprovals()}
            className="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors"
          >
            <AlertTriangle className="w-4 h-4" /> {counts.pending} Pending →
          </Link>
          <button className="flex items-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={k.label} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            ...
            <span
              className={`text-[10px] font-bold px-2 py-0.5 rounded-full mt-1 inline-block ${k.color}`}
            >
              {k.label}
            </span>
          </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {paged.length === 0 ? (
                <tr>
                  <td colSpan={14}>
                    <MarketplaceEmptyState title="No products found" icon={Package} />
                  </td>
                </tr>
              ) : (
                paged.map((p) => {
                  const pDiscount =
                    'discount' in p && typeof (p as any).discount === 'number'
                      ? (p as any).discount
                      : p.mrp > p.price
                        ? Math.round((1 - p.price / p.mrp) * 100)
                        : 0;
                  const pRating =
                    'rating' in p && typeof (p as any).rating === 'number'
                      ? (p as any).rating
                      : null;
                  const pReviews =
                    'reviews' in p && typeof (p as any).reviews === 'number'
                      ? (p as any).reviews
                      : null;
                  ...
                  return (
                    <tr
                      key={p.id}
                      className={`hover:bg-slate-50/50 transition-colors ${p.approval === 'rejected' ? 'opacity-60' : ''}`}
                    >
                      <td className="px-5 py-3.5">
                        ...
                        <p className="text-xs text-slate-400">
                          {p.images} images · {p.variants} variants
                        </p>
                      </td>
                          ...
                          <div className="flex flex-col items-center gap-0.5">
                            ...
                            {pReviews !== null && (
                              <span className="text-[9px] text-slate-400">
                                {pReviews > 999 ? `${(pReviews / 1000).toFixed(1)}k` : pReviews}
                              </span>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="px-4 py-3.5 text-center">
                        {pDelivery ? (
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <span
                              className={`text-[10px] font-medium ${pDelivery === 'Tomorrow' ? 'text-blue-600' : 'text-slate-500'}`}
                            >
                              {pDelivery}
                            </span>
                          </div>
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <span
                          className={`font-bold text-sm ${p.stock < 20 ? 'text-red-600' : 'text-slate-800'}`}
                        >
                      ...
                      <td className="px-4 py-3.5 text-center">
                        <p className="font-mono text-xs font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded">
                          {p.hsn}
                        </p>
                      ...
                      <td className="px-4 py-3.5 text-center">
                        <MarketplaceStatusBadge
                          status={p.approval === 'correction' ? 'correction' : (p.approval as any)}
                        />
                      </td>
                      <td className="px-4 py-3.5 text-center">
                        <button
                      ...
                      <td className="px-4 py-3.5 text-center">
                        ...
                        >
                          <Star
                            className={`w-4 h-4 ${p.featured ? 'text-amber-400 fill-amber-400' : 'text-slate-200'}`}
                          />
                        </button>
                      </td>
                      <td className="px-4 py-3.5 text-center">
                        <MarketplaceActionMenu
                          items={[
                            ...
                            ...(p.approval === 'pending' || p.approval === 'correction'
                              ...
                              : []),
                            {
                              label: p.visibility === 'published' ? 'Unpublish' : 'Publish',
                              icon: p.visibility === 'published' ? EyeOff : Eye,
                              onClick: () => toggleVisibility(p.id),
                              divider: true,
                            },
                            {
                              label: p.featured ? 'Remove from Featured' : 'Feature Product',
                              icon: Star,
                              onClick: () => toggleFeatured(p.id),
                            },
                            {
      ...
      {viewProduct && (
        <div className="fixed inset-0 z-50 flex">
          <div className="absolute inset-0 bg-black/40" onClick={() => setViewProduct(null)}>
            <DismissOnEscape onDismiss={() => setViewProduct(null)} />
          </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <MarketplaceStatusBadge
                      status={
                        viewProduct.approval === 'correction'
                          ? 'correction'
                          : (viewProduct.approval as any)
                      }
                    />
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded border ${viewProduct.visibility === 'published' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}
                    >
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    {viewProduct.discount > 0 ? (
                      <span className="inline-block mt-1 bg-red-50 text-red-600 text-xs font-bold px-2 py-0.5 rounded-full border border-red-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-xl p-4">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    <CountryFlag code={COUNTRY_TO_CODE[viewProduct.country] || 'QA'} size="xs" />{' '}
                    {viewProduct.country}
                  </p>
                </div>
                <div className="bg-slate-50 rounded-xl p-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <div className="flex items-center justify-center gap-1">
                    ...
                    <span className="text-sm font-bold text-slate-900">{viewProduct.rating}</span>
                  </div>
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    {viewProduct.reviews > 999
                      ? `${(viewProduct.reviews / 1000).toFixed(1)}k`
                      : viewProduct.reviews}{' '}
                    reviews
                  </p>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <p
                    className={`text-sm font-bold ${viewProduct.stock === 0 ? 'text-red-600' : viewProduct.stock < 20 ? 'text-amber-600' : 'text-slate-900'}`}
                  >
                    {viewProduct.stock}
                  </p>
              ...
              <div className="bg-blue-50 rounded-xl p-4">
                ...
                <div className="flex items-center gap-4">
                  <div>
                    <p className="text-[10px] text-blue-500 uppercase font-bold">HSN Code</p>
                    <p className="font-mono text-lg font-black text-blue-800">
                      {viewProduct.hsn || '—'}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-blue-500 uppercase font-bold">GST Rate</p>
                    <p className="text-lg font-black text-blue-800">{viewProduct.gst || '—'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-xl p-4 text-xs text-slate-500 space-y-1">
                <p>
                  <span className="font-bold">Created:</span> {viewProduct.created}
                </p>
                <p>
                  <span className="font-bold">Images:</span> {viewProduct.images} ·{' '}
                  <span className="font-bold">Variants:</span> {viewProduct.variants}
                </p>
                {viewProduct.featured && (
                  <p className="text-amber-600 font-bold">â­ Featured Product</p>
                )}
              </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    toggleVisibility(viewProduct.id);
                    setViewProduct({
                      ...viewProduct,
                      visibility:
                        viewProduct.visibility === 'published' ? 'unpublished' : 'published',
                    });
                  }}
                  className="flex-1 bg-white border border-slate-200 text-slate-700 font-bold py-2.5 rounded-lg text-sm hover:bg-slate-50 transition-colors flex items-center justify-center gap-2"
                >
                  {viewProduct.visibility === 'published' ? (
                    <>
                ...
                <button
                  onClick={() => {
                    toggleFeatured(viewProduct.id);
                    setViewProduct({ ...viewProduct, featured: !viewProduct.featured });
                  }}
                  className="flex-1 bg-white border border-slate-200 text-slate-700 font-bold py-2.5 rounded-lg text-sm hover:bg-slate-50 transition-colors flex items-center justify-center gap-2"
                >
                  <Star
                    className={`w-4 h-4 ${viewProduct.featured ? 'text-amber-400 fill-amber-400' : ''}`}
                  />
                  {viewProduct.featured ? 'Unfeature' : 'Feature'}
                </button>
                {viewProduct.approval !== 'rejected' && (
                  <button
                    onClick={() => {
                      setViewProduct(null);
                      setModal({
                    ...
                    className="bg-red-50 border border-red-200 text-red-600 font-bold py-2.5 px-4 rounded-lg text-sm hover:bg-red-100 transition-colors"
                  >
                    Suspend
                  </button>
                )}
