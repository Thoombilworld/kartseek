...
type ReturnItem = {
  ...
  status: 'Requested' | 'Approved' | 'Rejected' | 'Picked Up' | 'Refund Initiated' | 'Completed';
  amount: number;
  date: string;
  images: string[];
  customerNote: string;
...
const RETURNS: ReturnItem[] = [
  {
    ...
    date: '2026-06-05',
    images: ['📸', '📸'],
    customerNote: 'Screen flickers intermittently when brightness is above 80%.',
    sellerNote: '',
  },
  {
    id: 'RET-002',
    ...
    date: '2026-06-04',
    images: ['📸'],
    customerNote: 'Received Galaxy Buds2 instead of Buds3 Pro.',
    sellerNote: 'Confirmed — sending replacement.',
  },
  {
    id: 'RET-003',
    order: 'ORD-8820003',
    product: 'Anker PowerCore',
    ...
    date: '2026-06-03',
    images: [],
    customerNote: 'Capacity is 10000mAh, listed as 20000mAh.',
    sellerNote: 'Listing error — full refund approved.',
  },
  {
    id: 'RET-004',
    order: 'ORD-8820005',
    ...
    date: '2026-06-01',
    images: [],
    customerNote: 'Ordered US 8, received US 9.',
    sellerNote: 'Replacement shipped — DL9876543.',
  },
  {
    id: 'RET-005',
    order: 'ORD-8820007',
    ...
    date: '2026-06-02',
    images: ['📸', '📸', '📸'],
    customerNote: 'Ripped at the seam. Clearly a manufacturing defect.',
    sellerNote: 'Quality check confirmed. Refund processing.',
  },
  {
    id: 'RET-006',
    order: 'ORD-9001',
    ...
    sellerNote: 'Non-returnable category — electronics opened.',
  },
  {
    id: 'RET-007',
    order: 'ORD-9101',
    product: 'Thobe Premium',
    ...
    date: '2026-06-05',
    images: ['📸'],
    customerNote: 'Ordered white, received cream.',
    sellerNote: '',
  },
];
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-left">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-lg font-black text-slate-900">{ret.id}</h2>
            <p className="text-xs text-slate-500 mt-0.5">{ret.date}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-2">
            <MarketplaceStatusBadge
              status={STATUS_MAP[ret.status]?.status || 'pending'}
              customLabel={ret.status}
            />
            <span
              className={`text-[10px] font-bold px-2.5 py-1 rounded-md ${ret.type === 'Return' ? 'bg-orange-50 text-orange-700' : 'bg-blue-50 text-blue-700'}`}
            >
              {ret.type}
            </span>
          </div>
          ...
          <div className="bg-slate-50 rounded-xl p-4">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Order: <span className="text-blue-600 font-bold">{ret.order}</span>
            </p>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-sm font-bold text-amber-900">Return Reason</h3>
            </div>
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 mb-2 flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-slate-400" /> Customer Note
            </h3>
          ...
          {ret.images.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-slate-900 mb-2 flex items-center gap-2">
                <Camera className="w-4 h-4 text-slate-400" /> Evidence ({ret.images.length})
              </h3>
              <div className="flex gap-2">
          ...
          {ret.sellerNote && (
            <div>
              ...
              <p className="text-sm text-slate-700 bg-blue-50 rounded-xl p-3 border border-blue-200">
                {ret.sellerNote}
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <p className="text-sm font-bold text-slate-900">{ret.customer}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-xs text-slate-500">Seller</p>
              <p className="text-sm font-bold text-slate-900">{ret.seller}</p>
            </div>
          </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-black text-slate-900">Reject Return</h3>
        </div>
        <p className="text-sm text-slate-600 mb-4">
          Provide a reason for rejecting <span className="font-bold">{returnId}</span>.
        </p>
        <select
          value={reason}
...
export default function ReturnsPage() {
  ...
  const {
    ...
    () => adminMarketplaceApi.getReturns({ status: statusFilter || undefined, country }),
  ...
  const {
    ...
    isFiltered,
    formatCurrencyValue,
  } = useMarketplaceRegionFilter(RETURNS);
  ...
  const kpis = [
    {
      label: 'Pending',
      val: regionFiltered.filter((r) => r.status === 'Requested').length,
      color: 'text-amber-600',
    },
    {
      label: 'Approved',
      val: regionFiltered.filter((r) => r.status === 'Approved' || r.status === 'Picked Up').length,
      color: 'text-blue-600',
    },
    {
      label: 'Completed',
      val: regionFiltered.filter((r) => r.status === 'Completed' || r.status === 'Refund Initiated')
        .length,
      color: 'text-emerald-600',
    },
    {
      label: 'Rejected',
      val: regionFiltered.filter((r) => r.status === 'Rejected').length,
      color: 'text-red-600',
    },
  ];
  ...
  const handleApprove = (ret: ReturnItem) => {
    execute(
      () => adminMarketplaceApi.approveReturn(ret.id),
  ...
  const handleReject = (reason: string) => {
    ...
    execute(
      () => adminMarketplaceApi.rejectReturn(rejectReturn.id, { reason }),
      `Return ${rejectReturn.id} rejected`,
      () => refetch(),
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {isFiltered ? `${regionLabel} — ` : ''}Manage product returns and replacements
          </p>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {paged.length === 0 ? (
              <tr>
                <td colSpan={9}>
                  <MarketplaceEmptyState title="No returns found" icon={RotateCcw} />
                </td>
              </tr>
            ) : (
              paged.map((r) => (
                <tr
                  ...
                  tabIndex={0}
                  onKeyDown={activateOnKey(() => setSelectedReturn(r))}
                >
                  <td className="px-4 py-3.5">
                    <p className="font-mono font-bold text-blue-700 text-xs">{r.id}</p>
                    <p className="text-[10px] text-slate-400">{r.order}</p>
                  </td>
                  <td className="px-4 py-3.5 text-sm font-medium text-slate-800 max-w-[160px] truncate">
                    {r.product}
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${r.type === 'Return' ? 'bg-orange-50 text-orange-700' : 'bg-blue-50 text-blue-700'}`}
                    >
                      {r.type}
                    </span>
                  </td>
                  <td className="px-4 py-3.5 text-center">
                    <MarketplaceStatusBadge
                      status={STATUS_MAP[r.status]?.status || 'pending'}
      ...
      {selectedReturn && (
        <ReturnDetailDrawer
          ...
          onReject={() => setRejectReturn(selectedReturn)}
        />
      )}
      {rejectReturn && (
        <RejectModal
          returnId={rejectReturn.id}
