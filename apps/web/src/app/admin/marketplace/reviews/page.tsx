...
type Review = {
  ...
  text: string;
  seller: string;
  status: 'Published' | 'Flagged' | 'Under Review' | 'Removed';
  date: string;
  reports: number;
  helpful: number;
  images: string[];
...
const REVIEWS: Review[] = [
  {
    ...
    rating: 5,
    title: 'Amazing phone!',
    text: 'Best phone I have ever used. Camera quality is outstanding. The titanium frame feels premium and the USB-C is finally here. Battery easily lasts a full day with heavy usage.',
  ...
  },
  {
    ...
    text: 'Audio is fantastic but a bit tight on my head. ANC is best in class. Multipoint connection works flawlessly.',
    seller: 'Sony Store',
    status: 'Published',
    date: '2026-06-05',
    reports: 0,
  ...
  {
    id: 'REV-003',
    product: 'Cheap Power Bank',
    customer: 'Anonymous',
    country: 'India',
    rating: 1,
    title: 'FAKE PRODUCT!!!',
    text: 'This is a counterfeit product. Seller is fraud. DO NOT BUY. The capacity is nowhere close to what is listed. Stopped working after 2 days.',
  ...
  },
  {
    ...
    text: 'Superb performance and battery life. Highly recommended for professionals and students alike.',
    seller: 'Apple India Store',
    status: 'Published',
    date: '2026-06-04',
    reports: 0,
  ...
  },
  {
    ...
    country: 'India',
    rating: 2,
    title: 'Competitor spam',
    text: 'Buy iPhone instead this is terrible phone worst purchase ever made avoid at all costs Samsung is the worst company',
  ...
  },
  {
    ...
    rating: 5,
    title: 'Best foldable!',
    text: 'Amazing innovation. Worth the premium. The inner display is gorgeous and multitasking is next level.',
  ...
  },
  {
    ...
    text: 'Great suction power. Excellent build quality. A bit heavy for extended use but results are incredible.',
  ...
  },
  {
    ...
    country: 'India',
    rating: 3,
    title: 'Wrong color received',
    text: 'Ordered black, received navy blue. Product itself is fine but the color mismatch is disappointing.',
    seller: 'Nike India',
    status: 'Under Review',
    date: '2026-06-03',
    reports: 0,
  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={`${sz} ${i < rating ? 'text-amber-500 fill-amber-500' : 'text-slate-200'}`}
        />
      ))}
    </div>
  );
      ...
      <div className="relative w-full max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-left">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            {r.reports > 0 && (
              <span className="text-[10px] font-bold text-red-600 flex items-center gap-0.5 bg-red-50 px-2 py-1 rounded-md">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {r.images.map((img, i) => (
                  <div
                    key={i}
                    className="w-20 h-20 bg-slate-100 rounded-xl flex items-center justify-center text-2xl border border-slate-200"
                  >
                    {img}
                  </div>
                ))}
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-black text-slate-900">Remove Review</h3>
        </div>
        <p className="text-sm text-slate-600 mb-4">
          This will hide <span className="font-bold">{reviewId}</span> from customers.
        </p>
        <select
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          className="w-full border border-slate-200 rounded-xl p-3 text-sm mb-4 outline-none focus:ring-2 focus:ring-red-200"
        >
...
export default function AdminReviewsPage() {
  ...
  const {
    ...
    () =>
      adminMarketplaceApi.getReviews({
        status: filter !== 'all' ? filter : undefined,
        rating: ratingFilter || undefined,
        country,
      }),
    [filter, ratingFilter, country],
  );
  ...
  const avgRating = regionFiltered.length
    ? (regionFiltered.reduce((a, r) => a + r.rating, 0) / regionFiltered.length).toFixed(1)
    : '0';

  const handleApprove = (r: Review) => {
    execute(
      () => adminMarketplaceApi.flagReview(r.id, 'approved'),
  ...
  const handleFlag = (r: Review) => {
    execute(
      () => adminMarketplaceApi.flagReview(r.id, 'flagged'),
      `Review ${r.id} flagged for review`,
      () => refetch(),
    );
    setSelectedReview(null);
  };
  const handleRemove = (reason: string) => {
    ...
    execute(
      () => adminMarketplaceApi.hideReview(removeReview.id),
      `Review ${removeReview.id} removed`,
      () => refetch(),
    );
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {isFiltered ? `${regionLabel} — ` : ''}Moderate customer reviews and manage reported
            content
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-2xl font-black text-slate-900">{regionFiltered.length}</p>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-2xl font-black text-emerald-600">
            {regionFiltered.filter((r) => r.status === 'Published').length}
          </p>
          <p className="text-xs text-slate-500 mt-1">Published</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-2xl font-black text-red-600">
            {regionFiltered.filter((r) => r.status === 'Flagged').length}
          </p>
          <p className="text-xs text-slate-500 mt-1">Flagged</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-2xl font-black text-amber-600">
            {regionFiltered.filter((r) => r.status === 'Under Review').length}
          </p>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search product, customer, or review title..."
        ...
        <div className="flex gap-2">
          {['all', 'Published', 'Flagged', 'Under Review'].map((s) => (
            <button
              key={s}
              onClick={() => {
                setFilter(s);
        ...
        <div className="flex gap-1">
          {[0, 1, 2, 3, 4, 5].map((r) => (
            <button
              ...
              className={`px-2 py-2 text-xs font-bold rounded-xl border transition-colors ${ratingFilter === r ? 'bg-amber-600 text-white border-amber-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
            >
              {r === 0 ? '★' : `${r}★`}
            </button>
          ))}
        ...
        {paged.length === 0 ? (
          ...
          paged.map((r) => (
            <div
              ...
              role="button"
              tabIndex={0}
              onKeyDown={activateOnKey(() => setSelectedReview(r))}
              className={`bg-white border rounded-2xl p-5 cursor-pointer hover:shadow-md transition-shadow ${r.status === 'Flagged' ? 'border-red-200 bg-red-50/20' : 'border-slate-200'}`}
            >
              <div className="flex items-start gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${STATUS_STYLES[r.status]}`}
                    >
                      {r.status}
                    </span>
                    {r.reports > 0 && (
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-slate-500">
                      by {r.customer} ·{' '}
                      <CountryFlag code={COUNTRY_TO_CODE[r.country] || 'IN'} size="sm" /> · {r.date}
                    </span>
                  </div>
                  ...
                  <p className="text-xs text-slate-400 mt-2">
                    Seller: {r.seller} · {r.id} · {r.helpful > 0 ? `👍 ${r.helpful} helpful` : ''}
                  </p>
                </div>
                <div
                ...
                >
                  {r.status !== 'Published' && (
                    <button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg flex items-center gap-1 transition-colors"
                    >
                      <ThumbsUp className="w-3 h-3" />
                      Approve
                    </button>
                  ...
                  {r.status !== 'Removed' && (
                    <button
                      onClick={() => setRemoveReview(r)}
                      className="bg-red-50 hover:bg-red-100 text-red-600 text-[10px] font-bold px-2.5 py-1.5 rounded-lg border border-red-200 flex items-center gap-1 transition-colors"
                    >
      ...
      {selectedReview && (
        <ReviewDrawer
          ...
          onRemove={() => setRemoveReview(selectedReview)}
          onFlag={() => handleFlag(selectedReview)}
        />
      )}
      {removeReview && (
        <RemoveModal
          reviewId={removeReview.id}
          onConfirm={handleRemove}
          onClose={() => setRemoveReview(null)}
        />
      )}
