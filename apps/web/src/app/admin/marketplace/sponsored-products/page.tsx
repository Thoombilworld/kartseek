    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl overflow-y-auto animate-slide-left">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-lg font-black text-slate-900">{s.product}</h2>
        ...
        <div className="p-6 space-y-5">
          <div className="flex gap-2">
            ...
            <span className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded-md">
              <CountryFlag code={COUNTRY_TO_CODE[s.country] || 'IN'} size="sm" /> {s.country}
            </span>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl p-5 text-white">
            <div className="flex justify-between items-end">
              <div>
                <p className="text-sm font-bold opacity-80">Budget</p>
                <p className="text-2xl font-black">
                  {formatCurrency(s.spent)} / {formatCurrency(s.budget)}
                </p>
              </div>
              <p className="text-3xl font-black opacity-60">{budgetPct}%</p>
            </div>
            <div className="h-2 bg-white/20 rounded-full mt-3 overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${budgetPct > 90 ? 'bg-red-400' : budgetPct > 60 ? 'bg-amber-400' : 'bg-emerald-400'}`}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-blue-50 rounded-xl p-3">
              <p className="text-[10px] text-blue-600">Cost Per Click</p>
              <p className="text-lg font-black text-blue-700">{formatCurrency(s.cpc)}</p>
            </div>
            <div className="bg-emerald-50 rounded-xl p-3">
              <p className="text-[10px] text-emerald-600">Click-Through Rate</p>
              <p className="text-lg font-black text-emerald-700">{s.ctr}%</p>
            </div>
          </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-black text-slate-900">Reject Ad</h3>
        </div>
        <p className="text-sm text-slate-600 mb-4">{itemId}</p>
        <select
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          className="w-full border border-slate-200 rounded-xl p-3 text-sm mb-4 outline-none"
        >
...
export default function SponsoredProductsPage() {
  ...
  const {
    ...
    () =>
      adminMarketplaceApi.getSponsoredProducts({
  ...
  const {
    ...
    isFiltered,
    formatCurrencyValue,
  } = useMarketplaceRegionFilter(SPONSORED);
  ...
  const handleApprove = (s: SponsoredProduct) => {
    execute(
      () => adminMarketplaceApi.updateSponsoredProduct(s.id, { status: 'Active' }),
      `${s.id} approved`,
      () => refetch(),
    );
    setSelected(null);
  ...
  const handleReject = (reason: string) => {
    ...
    execute(
      () =>
        adminMarketplaceApi.updateSponsoredProduct(rejectItem.id, { status: 'Rejected', reason }),
      `${rejectItem.id} rejected`,
      () => refetch(),
    );
    setRejectItem(null);
    setSelected(null);
  };
  const handlePause = (s: SponsoredProduct) => {
    execute(
      () => adminMarketplaceApi.updateSponsoredProduct(s.id, { status: 'Paused' }),
      `${s.id} paused`,
      () => refetch(),
    );
    setSelected(null);
  };
  const handleResume = (s: SponsoredProduct) => {
    execute(
      () => adminMarketplaceApi.updateSponsoredProduct(s.id, { status: 'Active' }),
      `${s.id} resumed`,
      () => refetch(),
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Sponsored Products</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            {isFiltered ? `${regionLabel} — ` : ''}Manage seller advertising campaigns and ad
            placements
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          {
            l: 'Active Ads',
            v: regionFiltered.filter((s) => s.status === 'Active').length,
            c: 'text-emerald-600',
          },
          {
            l: 'Pending',
            v: regionFiltered.filter((s) => s.status === 'Pending').length,
            c: 'text-amber-600',
          },
          { l: 'Total Spend', v: fmt(totalSpent), c: 'text-blue-600' },
          { l: 'Impressions', v: (totalImpressions / 1000).toFixed(0) + 'K', c: 'text-purple-600' },
          {
            l: 'Avg ROAS',
            v: regionFiltered.filter((s) => s.roas > 0).length
              ? (
                  regionFiltered.reduce((a, s) => a + s.roas, 0) /
                  regionFiltered.filter((s) => s.roas > 0).length
                ).toFixed(1) + 'x'
              : '0x',
            c: 'text-slate-900',
          },
      ...
      <div className="flex flex-col md:flex-row gap-3">
        ...
        <div className="flex gap-2">
          {['all', 'Active', 'Pending', 'Paused', 'Rejected', 'Ended'].map((s) => (
            <button
              key={s}
              onClick={() => {
                setFilter(s);
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {paged.length === 0 ? (
              <tr>
                <td colSpan={8}>
                  <MarketplaceEmptyState title="No sponsored products found" icon={Target} />
                </td>
              </tr>
            ) : (
              paged.map((s) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <span className="text-xs font-bold">{fmt(s.spent)}</span>
                    <span className="text-[10px] text-slate-400">/{fmt(s.budget)}</span>
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <span
                      className={`font-bold ${s.ctr >= 3 ? 'text-emerald-600' : s.ctr >= 2 ? 'text-amber-600' : 'text-red-600'}`}
                    >
                      {s.ctr}%
                    </span>
                  </td>
                  <td className="px-4 py-3.5 text-right">
                    <span
                      className={`font-bold ${s.roas >= 8 ? 'text-emerald-600' : s.roas >= 4 ? 'text-amber-600' : 'text-slate-600'}`}
                    >
                      {s.roas}x
                    </span>
                  </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2">
          <p className="text-xs text-slate-500">{filtered.length} ads</p>
      ...
      {selected && (
        <SponsoredDrawer
          ...
          onReject={() => setRejectItem(selected)}
