...
/**
 * Grocery Products — franchise view.
 *
...
function Content({ franchiseId }: { franchiseId: string }) {
  ...
  const { data, loading, error, reload } = useAsyncData<any>(
    () => franchiseGroceryApi.getProducts(franchiseId, { search: search || undefined }),
    [franchiseId, search],
  );

  const rows: any[] = (data as any)?.products ?? [];
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Package className="w-6 h-6 text-green-400" /> Grocery Products
          </h1>
          <p className="text-slate-400 mt-1">Catalogue across stores in your franchise zone</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700/50 text-left">
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Product</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Category</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Brand</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Price</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Listed</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700/30">
              ...
              {filtered.map((row, i) => (
                <tr key={row.id ?? i} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-sm text-slate-300">{row.brand ?? '—'}</td>
                  <td className="px-5 py-4 text-sm text-slate-300">{row.weightVariants?.[0] ? formatPrice(Number(row.weightVariants[0].price)) : '—'}</td>
                  <td className="px-5 py-4 text-sm text-slate-300">{row.isAvailable ? 'Yes' : 'No'}</td>
                </tr>
              ))}
            </tbody>
          </table>
