...
function Content({ franchiseId }: { franchiseId: string }) {
  ...
  const { data, loading, error, reload } = useAsyncData<any>(
    () => franchiseGroceryApi.getStores(franchiseId, { search: search || undefined }),
    [franchiseId, search],
  );

  const rows: any[] = data?.stores ?? [];
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Store className="w-6 h-6 text-green-400" /> Grocery Stores
          </h1>
          <p className="text-slate-400 mt-1">Stores operating in your franchise zone</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700/50 text-left">
                ...
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Address</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Rating</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Orders</th>
            ...
            <tbody className="divide-y divide-slate-700/30">
              ...
              {filtered.map((row, i) => (
                <tr key={row.id ?? i} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-sm text-slate-300">{row.address ?? '—'}</td>
                  <td className="px-5 py-4 text-sm text-slate-300">{Number(row.rating ?? 0) > 0 ? Number(row.rating).toFixed(1) : '—'}</td>
                  <td className="px-5 py-4 text-sm text-slate-300">{Number(row.totalOrders ?? 0).toLocaleString()}</td>
                  <td className="px-5 py-4 text-sm">
                    <span className={`text-xs font-bold px-2 py-1 rounded ${row.status === 'APPROVED' ? 'bg-emerald-500/10 text-emerald-400' : row.status === 'SUSPENDED' ? 'bg-red-500/10 text-red-400' : 'bg-amber-500/10 text-amber-400'}`}>
                      {row.status}
                    </span>
                  </td>
                </tr>
