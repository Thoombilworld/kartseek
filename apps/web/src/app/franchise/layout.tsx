...
const navSections = [
  {
    label: 'Overview',
    items: [
      { href: '/franchise', label: 'Dashboard', icon: LayoutDashboard, exact: true },
      { href: '/franchise/analytics', label: 'Analytics & Reports', icon: BarChart3 },
    ],
  },
  {
    label: 'Modules',
    items: [
      { href: '/franchise/grocery', label: 'Grocery', icon: ShoppingCart },
      { href: '/franchise/restaurant', label: 'Restaurant', icon: UtensilsCrossed },
      { href: '/franchise/pharmacy', label: 'Pharmacy', icon: Pill },
      { href: '/franchise/marketplace', label: 'Marketplace', icon: Store },
      { href: '/franchise/taxi', label: 'Taxi & Rides', icon: Car },
      { href: '/franchise/doctor', label: 'Doctor & Clinics', icon: Stethoscope },
      { href: '/franchise/hotel-booking', label: 'Hotel Booking', icon: Landmark },
    ],
  },
  {
    label: 'Operations',
    items: [
      { href: '/franchise/orders', label: 'All Orders', icon: Package },
      { href: '/franchise/vendors', label: 'Local Vendors', icon: Store },
      { href: '/franchise/delivery-partners', label: 'Delivery Partners', icon: Truck },
      { href: '/franchise/zones', label: 'Delivery Zones', icon: MapPin },
      { href: '/franchise/customers', label: 'Customers', icon: Users },
    ],
  },
  {
    label: 'Finance',
    items: [
      { href: '/franchise/commissions', label: 'Commissions', icon: Landmark },
      { href: '/franchise/payouts', label: 'Payouts', icon: DollarSign },
    ],
  },
  {
    label: 'Management',
    items: [
      { href: '/franchise/marketing', label: 'Marketing & Promos', icon: Megaphone },
      { href: '/franchise/support', label: 'Support Tickets', icon: HeadphonesIcon },
      { href: '/franchise/staff', label: 'Staff & Roles', icon: UserCog },
    ],
  },
  {
    label: 'System',
...
const PUBLIC_ROUTES = ['/franchise/login', '/franchise/opportunity'];
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white">
        <div className="text-center max-w-md px-6">
          ...
          <div className="flex flex-col gap-3">
            <Link href={'/franchise/login?redirect=' + encodeURIComponent(pathname)}
            ...
            <Link href="/" className="text-slate-600 hover:text-slate-400 text-xs transition-colors mt-2">
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="h-16 flex items-center px-6 border-b border-slate-800 bg-slate-950 shrink-0">
        <Link href="/franchise" className="font-bold text-lg text-white tracking-wide">
              ...
              {showNotifications && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                    ...
                    <Link href="/franchise/support" onClick={() => setShowNotifications(false)}
              ...
              {showProfile && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                    ...
                    <div className="py-1">
                      <Link href="/franchise/settings" onClick={() => setShowProfile(false)}
                        className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                        <Settings className="w-4 h-4 text-slate-400" /> Account Settings
                      </Link>
                      <Link href="/franchise/staff" onClick={() => setShowProfile(false)}
                        className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
