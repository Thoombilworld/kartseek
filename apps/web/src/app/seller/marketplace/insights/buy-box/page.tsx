...
/**
 * Buy Box Insights — not implemented on the backend.
 *
 * This page used to render hard-coded rows as though they were the seller's own
 * figures. There is no `GET /sellers/:id/analytics/buy-box` on the gateway, so nothing here was ever
 * real, and a seller reading it could not tell. Until the endpoint exists the
 * page says so rather than inventing an answer.
 */
export default function BuyBoxInsightsPage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
          <Award className="w-6 h-6 text-slate-400" aria-hidden />
          Buy Box Insights
        </h1>
        <p className="text-sm text-slate-500 mt-1">Where you win and lose the buy box</p>
      </div>

      <SellerUnavailable
        feature="Buy Box Insights"
