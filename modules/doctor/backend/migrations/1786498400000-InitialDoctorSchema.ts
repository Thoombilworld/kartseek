 ...
 *
 * The first two statements are `CREATE SCHEMA IF NOT EXISTS "doctor"` and
 ...
 * only because the ledger lives in `public.doctor_migrations` rather than inside
 * this schema — TypeORM builds the ledger before the first `up()` runs, so a
 * ledger in `doctor` would need the schema that this line creates.
 *
...
export class InitialDoctorSchema1786498400000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // The ledger is deliberately `public.doctor_migrations` (see
    // data-source.ts), so TypeORM does not need this schema to exist
    // before this line runs.
    await queryRunner.query(`CREATE SCHEMA IF NOT EXISTS "doctor"`);
...
END $guard$`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DO $guard$ BEGIN
  DROP SCHEMA IF EXISTS "doctor" RESTRICT;
