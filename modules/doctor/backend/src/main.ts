...
async function bootstrap() {
  ...
  // ── TCP transport ───────────────────────────────────────────────────────────
  const tcpPort = +(process.env.DOCTOR_TCP_PORT ?? 4007);
  ...
  const httpPort = +(process.env.DOCTOR_SERVICE_PORT ?? 3017);
  await app.listen(httpPort);
  Logger.log(`🩺 Doctor Service — HTTP :${httpPort} | TCP :${tcpPort}`, 'Bootstrap');
}
bootstrap();
