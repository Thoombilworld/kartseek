/**
 * Where the grocery module's database is — resolved in exactly one place.
 *
 * `data-source.ts` (the TypeORM CLI's runner) and `grocery-service.module.ts` (the
 * running service) both call `resolveGroceryDbConfig`. They used to resolve the
...
export const GROCERY_DB_SCHEMA = 'grocery';

/**
 * The migration ledger, in `public` and named per module.
 *
 * NOT `grocery.migrations`. TypeORM builds the ledger table inside the DataSource's
 * `schema` and does it *before* the first migration's `up()` runs, so on a fresh
 * dedicated database — where the `grocery` schema does not exist yet — `migration:run`
 * died on `CREATE TABLE "grocery"."migrations"` before anything could create it. The
 ...
 */
export const GROCERY_MIGRATIONS_TABLE = 'grocery_migrations';

/** Everything needed to reach the grocery database, with nothing left implicit. */
export interface ResolvedGroceryDb {
  host: string;
  port: number;
  username: string;
  password: string;
