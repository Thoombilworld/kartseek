/**
 * Where the hotel module's database is — resolved in exactly one place.
 *
 * `data-source.ts` (the TypeORM CLI's runner) and `hotel-service.module.ts` (the
 * running service) both call `resolveHotelDbConfig`. They used to resolve the
...
export const HOTEL_DB_SCHEMA = 'hotel';

/**
 * The migration ledger, in `public` and named per module.
 *
 * NOT `hotel.migrations`. TypeORM builds the ledger table inside the DataSource's
 * `schema` and does it *before* the first migration's `up()` runs, so on a fresh
 * dedicated database — where the `hotel` schema does not exist yet — `migration:run`
 * died on `CREATE TABLE "hotel"."migrations"` before anything could create it. The
 ...
 */
export const HOTEL_MIGRATIONS_TABLE = 'hotel_migrations';

/** Everything needed to reach the hotel database, with nothing left implicit. */
export interface ResolvedHotelDb {
  host: string;
  port: number;
  username: string;
  password: string;
...
/**
 * `HOTEL_DB_*` wins, the shared `DB_*` answers next, and the last resorts are the
 * shared platform database — the same order and the same values on both sides.
 */
export function resolveHotelDbConfig(read: EnvReader): ResolvedHotelDb {
  ...
  return {
    host: first('HOTEL_DB_HOST', 'DB_HOST') ?? 'localhost',
    port: Number(first('HOTEL_DB_PORT', 'DB_PORT') ?? 5432),
    username: first('HOTEL_DB_USER', 'DB_USER') ?? 'postgres',
    ...
    password: first('HOTEL_DB_PASSWORD', 'DB_PASSWORD', 'DB_PASS') ?? 'kartseek123',
    database: first('HOTEL_DB_NAME', 'DB_NAME') ?? 'kartseek_db',
  };
}
