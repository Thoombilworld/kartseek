...
export interface HotelRequester { requesterId?: string; requesterRole?: string }
...
import { WebhookPaymentDto, WebhookRefundDto, PaymentWebhookStatus } from './dto/webhook-payment.dto';
...
export class HotelService {
  ...
  constructor(
    ...
    @InjectRepository(HotelSeasonalPricing) private readonly seasonalRepo: Repository<HotelSeasonalPricing>,
    private readonly redis: RedisService,
    private readonly kafka: KafkaProducerService,
  ) {}

  async healthCheck() {
    const dbOk = await this.hotelRepo.query('SELECT 1').then(() => true).catch(() => false);
    return { service: 'hotel-service', status: dbOk ? 'ok' : 'degraded', db: dbOk, timestamp: new Date().toISOString() };
  }
  ...
  async searchHotels(dto: SearchHotelsDto) {
    ...
    if (dto.q) {
      qb.andWhere('(hotel.name ILIKE :q OR hotel.city ILIKE :q OR hotel.address ILIKE :q)', { q: `%${dto.q}%` });
    }
    if (dto.city) {
      qb.andWhere('hotel.city ILIKE :city', { city: `%${dto.city}%` });
    }
    if (dto.starRating) {
      const stars = dto.starRating.split(',').map(Number).filter(n => !isNaN(n));
    ...
    if (dto.lat && dto.lng && dto.radius) {
      ...
      qb.andWhere('hotel.latitude BETWEEN :latMin AND :latMax', { latMin: dto.lat - radiusDeg, latMax: dto.lat + radiusDeg });
      qb.andWhere('hotel.longitude BETWEEN :lngMin AND :lngMax', { lngMin: dto.lng - radiusDeg, lngMax: dto.lng + radiusDeg });
    }

    // Sort
    switch (dto.sort) {
      case HotelSortBy.PRICE_ASC: qb.orderBy('room.pricePerNight', 'ASC'); break;
      case HotelSortBy.PRICE_DESC: qb.orderBy('room.pricePerNight', 'DESC'); break;
      case HotelSortBy.RATING_DESC: qb.orderBy('hotel.rating', 'DESC'); break;
      case HotelSortBy.STARS_DESC: qb.orderBy('hotel.starRating', 'DESC'); break;
      case HotelSortBy.REVIEWS_DESC: qb.orderBy('hotel.reviewCount', 'DESC'); break;
      default: qb.orderBy('hotel.rating', 'DESC').addOrderBy('hotel.reviewCount', 'DESC'); break;
    }
  ...
  async getRoomAvailability(hotelId: string, checkin: string, checkout: string, guests: number) {
    ...
    const overlappingBookings = await this.bookingRepo.createQueryBuilder('b')
      .select('b.roomId, COUNT(*) as booked')
      .where('b.hotelId = :hotelId', { hotelId })
      .andWhere('b.status IN (:...activeStatuses)', { activeStatuses: ['CONFIRMED', 'CHECKED_IN', 'MODIFIED'] })
      .andWhere('b."checkinDate" < :checkout AND b."checkoutDate" > :checkin', { checkin, checkout })
      .groupBy('b.roomId')
      .getRawMany();
    ...
    return {
      hotelId, checkin, checkout, guests, nights,
      rooms: rooms
        .filter(r => r.maxGuests >= guests)
        .map(r => {
          const booked = bookedMap.get(r.id) || 0;
          const available = r.totalInventory - booked;
          const effectivePrice = this.calculateSeasonalPrice(Number(r.pricePerNight), r.id, checkin, checkout, seasonalRules);
          return {
            id: r.id, name: r.name, type: r.type, bedType: r.bedType, maxGuests: r.maxGuests,
  ...
  private calculateSeasonalPrice(basePrice: number, roomId: string, checkin: string, checkout: string, rules: HotelSeasonalPricing[]): number {
  ...
  async createBooking(hotelId: string, dto: CreateBookingDto) {
    ...
    const booking = this.bookingRepo.create({
      hotelId, customerId: dto.customerId, roomId: dto.roomId,
      bookingNumber, confirmationCode,
      checkinDate: dto.checkin, checkoutDate: dto.checkout, nights,
      adults: dto.guests || 2, roomCount: dto.rooms || 1,
      pricePerNight: nightlyRate, roomTotal: subtotal, taxAmount: taxesAndFees,
      serviceFee: 0, discount: walletDiscount + loyaltyDiscount,
      walletAmountUsed: walletDiscount, pointsRedeemed: dto.loyaltyPoints || 0,
      pointsDiscount: loyaltyDiscount,
      grandTotal, currency: room.currency || hotel.currency || 'AED',
      status: 'CONFIRMED', paymentMethod: dto.paymentMethod,
      specialRequests: dto.specialRequests,
      primaryGuest: { firstName: dto.guestName || '', lastName: '', email: dto.guestEmail || '', phone: dto.guestPhone || '' },
      hotelName: hotel.name, roomName: room.name, hotelCity: hotel.city, hotelCountryCode: hotel.countryCode,
    } as any);
    ...
    await this.kafka.publish('hotel.booking.created', { id: savedId, customerId: dto.customerId, hotelId });
  ...
  private async findBookingFor(id: string, requester?: HotelRequester) {
    const privileged = ['admin', 'super_admin', 'seller', 'hotel_owner']
      .includes(String(requester?.requesterRole ?? '').toLowerCase());
    ...
    const where = privileged
      ? { id }
      : { id, customerId: requester!.requesterId };
    const booking = await this.bookingRepo.findOne({ where, relations: { hotel: true, room: true } });
  ...
  async cancelBooking(id: string, reason: string, requester?: HotelRequester) {
    ...
    if (['CANCELLED', 'REFUNDED', 'CHECKED_OUT'].includes(booking.status)) {
      throw new BadRequestException(`Booking ${id} cannot be cancelled (status: ${booking.status})`);
    }
    ...
    return { success: true, bookingId: id, status: 'CANCELLED', refundAmount, currency: booking.currency, refundStatus: 'PROCESSING' };
  }
  ...
  async modifyBooking(id: string, dto: ModifyBookingDto, requester?: HotelRequester) {
    ...
    const modifications: Array<{ field: string; oldValue: string; newValue: string; modifiedAt: string; modifiedBy: string }> = [];

    if (dto.checkin && dto.checkin !== booking.checkinDate) {
      modifications.push({ field: 'checkinDate', oldValue: booking.checkinDate, newValue: dto.checkin, modifiedAt: new Date().toISOString(), modifiedBy: 'customer' });
      booking.checkinDate = dto.checkin;
    }
    if (dto.checkout && dto.checkout !== booking.checkoutDate) {
      modifications.push({ field: 'checkoutDate', oldValue: booking.checkoutDate, newValue: dto.checkout, modifiedAt: new Date().toISOString(), modifiedBy: 'customer' });
  ...
  async submitReview(hotelId: string, dto: SubmitReviewDto) {
    ...
    const review = this.reviewRepo.create({
      hotelId, customerId: dto.userId, customerName: 'Guest', bookingId: dto.bookingId,
      rating: dto.rating, comment: dto.comment,
      cleanlinessRating: dto.cleanliness, serviceRating: dto.service,
      valueRating: dto.value, locationRating: dto.location,
      amenitiesRating: dto.facilities,
      stayType: dto.stayType, isVisible: true,
    } as any);
    ...
    await this.hotelRepo.update(hotelId, { rating: parseFloat(avg) || 0, reviewCount: parseInt(count) || 0 });
  ...
  async getAllHotels(page = 1, limit = 20, status?: string) {
    const where: any = {};
  ...
  async getAdminAnalytics() {
    ...
    const revenueResult = await this.bookingRepo.createQueryBuilder('b')
      ...
      .where('b.status IN (:...statuses)', { statuses: ['CONFIRMED', 'CHECKED_IN', 'CHECKED_OUT', 'COMPLETED'] })
      .getRawOne();

    const cancelledBookings = await this.bookingRepo.count({ where: { status: 'CANCELLED' as any } });

    return {
      totalHotels, activeHotels,
      pendingApprovals: totalHotels - activeHotels,
      totalBookings, cancelledBookings,
      cancelRate: totalBookings > 0 ? `${((cancelledBookings / totalBookings) * 100).toFixed(1)}%` : '0%',
  ...
  async getAdminFraudAnalytics() {
    // Flag suspicious patterns
    const sameDayCancellations = await this.bookingRepo.createQueryBuilder('b')
      .select('b.customerId, COUNT(*) as cancelCount')
      .where('b.status = :status', { status: 'CANCELLED' })
      .andWhere('b.cancelledAt >= :since', { since: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) })
      .groupBy('b.customerId')
      .having('COUNT(*) > 3')
      .getRawMany();

    const highValueNoShows = await this.bookingRepo.createQueryBuilder('b')
      .where('b.status = :status AND b.grandTotal > :threshold', { status: 'NO_SHOW', threshold: 5000 })
      .getCount();
    ...
    return {
      ...
      riskScore: sameDayCancellations.length > 5 ? 'HIGH' : sameDayCancellations.length > 2 ? 'MEDIUM' : 'LOW',
    };
  }

  async getComplianceData() {
    const hotels = await this.hotelRepo.find({ select: { id: true, name: true, city: true, countryCode: true, licenseNumber: true, licenseExpiry: true, isAcceptingBookings: true } });
    const expiringLicenses = hotels.filter(h => {
      if (!h.licenseExpiry) return true;
      const daysUntilExpiry = (new Date(h.licenseExpiry).getTime() - Date.now()) / (1000 * 60 * 60 * 24);
      return daysUntilExpiry < 90;
    });
    return { totalHotels: hotels.length, expiringLicenses: expiringLicenses.length, hotels: expiringLicenses };
  }
  ...
  async registerHotelOwner(dto: RegisterOwnerDto) {
    const owner = this.ownerRepo.create({
      ...
      businessName: dto.businessName, countryCode: dto.country,
      taxNumber: dto.taxId, status: 'PENDING_VERIFICATION',
    } as any);
  ...
  async createHotel(ownerId: string, dto: CreateHotelDto) {
    const hotel = this.hotelRepo.create({
      ...dto, ownerId,
      status: 'PENDING_APPROVAL', isAcceptingBookings: false,
      latitude: dto.location?.lat, longitude: dto.location?.lng,
    } as any);
    ...
    if (dto.seasonalRules?.length) {
      for (const rule of dto.seasonalRules) {
        const seasonalPricing = this.seasonalRepo.create({
          hotelId: room.hotelId, roomId: room.id,
          name: rule.name, startDate: rule.startDate, endDate: rule.endDate,
          multiplier: rule.multiplier, fixedPrice: rule.fixedPrice,
          isActive: true,
        } as any);
  ...
  async getOwnerDashboard(ownerId: string) {
    ...
    const revenueResult = await this.bookingRepo.createQueryBuilder('b')
      ...
      .andWhere('b.status IN (:...statuses)', { statuses: ['CONFIRMED', 'CHECKED_IN', 'CHECKED_OUT', 'COMPLETED'] })
      .getRawOne();

    const reviewStats = await this.reviewRepo.createQueryBuilder('r')
      .select('AVG(r.rating)', 'avg')
      .addSelect('COUNT(*)', 'count')
      .where('r.hotelId IN (:...ids)', { ids: hotelIds })
      .getRawOne();

    const payoutResult = await this.payoutRepo.createQueryBuilder('p')
      .select('SUM(p.netAmount)', 'pending')
      .where('p.hotelId IN (:...ids) AND p.status = :status', { ids: hotelIds, status: 'PROCESSING' })
      .getRawOne();
  ...
  async processPaymentWebhook(dto: WebhookPaymentDto) {
    ...
    await this.kafka.publish('hotel.payment.processed', { bookingId: dto.bookingId, status: dto.status, transactionId: dto.transactionId });
  ...
  async processRefundWebhook(dto: WebhookRefundDto) {
    ...
    await this.bookingRepo.save(booking);
    await this.redis.del(`hotel:booking:${dto.bookingId}`);
    await this.kafka.publish('hotel.refund.processed', { bookingId: dto.bookingId, refundAmount: dto.refundAmount });
