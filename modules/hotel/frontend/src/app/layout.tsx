...
/**
 * Root layout for the hotel zone.
 *
...
// Carried over from the former app/hotel-booking/layout.tsx. Every route in this
// zone is hotel booking, so that file had no distinct level left to occupy once the
// routes moved: its metadata and its client wrapper belong to the zone root.
export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  ...moduleMeta('hotel-booking'),
};
...
export default async function HotelZoneLayout({ children }: { children: React.ReactNode }) {
  const cookieStore = await cookies();
  const headerStore = await headers();

  // Same resolution the shell performs, so a customer crossing from / into
  // /hotel-booking keeps their market and language rather than snapping back to the
    ...
    >
      ...
      <body className="bg-slate-50 min-h-screen text-slate-900 antialiased">
        {/*
          ...
          <main id="main-content"> that the hotel layout renders; it is not
        ...
        */}
        ...
        <AppShell
          ...
          country={country}
        >
          <HotelClientLayout>{children}</HotelClientLayout>
        </AppShell>
      </body>
    </html>
  );
