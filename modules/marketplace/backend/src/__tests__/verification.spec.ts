...
const EXPECTED_ENTITIES = [
  {
    name: 'Product',
    table: 'products',
    columns: ['id', 'name', 'slug', 'seller_id', 'mrp', 'status'],
  },
  {
    name: 'Seller',
    table: 'sellers',
    columns: ['id', 'businessName', 'storeSlug', 'verificationStatus', 'ownerId'],
  },
  ...
  { name: 'Brand', table: 'brands', columns: ['id', 'name', 'slug'] },
  {
    ...
    columns: ['id', 'productId', 'sellerId', 'sellingPrice'],
  },
  ...
  { name: 'WishlistItem', table: 'wishlist_items', columns: ['id', 'userId', 'productId'] },
  {
    ...
    columns: ['id', 'customerId', 'sellerId', 'grandTotal', 'status'],
  },
  {
    name: 'ReturnRequest',
    table: 'return_requests',
    columns: ['id', 'orderId', 'reason', 'status'],
  },
  ...
  { name: 'CouponUsage', table: 'coupon_usages', columns: ['id', 'couponId', 'customerId'] },
  {
    ...
    columns: ['id', 'orderId', 'status'],
  },
  {
    name: 'ProductVariant',
    table: 'product_variants',
    columns: ['id', 'productId', 'sku', 'sellingPrice', 'stock'],
  },
  {
    name: 'ProductQuestion',
    table: 'product_questions',
    columns: ['id', 'productId', 'questionText'],
  },
  { name: 'ProductAnswer', table: 'product_answers', columns: ['id', 'questionId', 'answerText'] },
  {
    name: 'DeliveryAssignment',
    table: 'delivery_assignments',
    columns: ['id', 'orderId', 'partnerId', 'status'],
  },
  {
    name: 'ProductAttribute',
    table: 'product_attributes',
    columns: ['id', 'productId', 'attributeName', 'attributeValue'],
  },
  {
    name: 'MarketplaceNotification',
    table: 'marketplace_notifications',
    columns: ['id', 'userId', 'message'],
  },
  ...
  describe('Entity/Table Registration', () => {
    it(`should have all ${EXPECTED_ENTITIES.length} entities registered in marketplace-service.module.ts`, () => {
      ...
      if (missingEntities.length > 0) {
        throw new Error(
          `Missing entities in marketplace-service.module.ts: ${missingEntities.join(', ')}`,
        );
      }
    ...
    it('should have entity files on disk for every expected entity', () => {
      ...
      const entityFiles = fs
        ...
        .filter((f: string) => f.endsWith('.entity.ts'));
    ...
    it('TypeORM config should target the "marketplace" schema', () => {
      ...
      const resolverSource = fs.readFileSync(
        path.resolve(__dirname, '..', 'db-config.ts'),
        'utf-8',
      );
      // The literal moved into `db-config.ts` with IN3, so that the service
      // connection and the migration runner name the same schema exactly once.
    ...
    it('every entity names the marketplace schema itself', () => {
      // The migration runner declares no connection-level schema — TypeORM would
      // otherwise build its ledger inside a schema that does not exist yet on a
      // fresh database — so an entity that does not name one would be created in
      // `public` by `migration:generate`.
      const entitiesDir = path.resolve(__dirname, '..', 'entities');
      const unqualified = fs
        .readdirSync(entitiesDir)
        .filter((f: string) => f.endsWith('.entity.ts'))
        .filter((f: string) => {
          const src = fs.readFileSync(path.join(entitiesDir, f), 'utf-8');
          const decorators = src.match(/@Entity\([^)]*\)/g) ?? [];
          return decorators.some((d: string) => !d.includes("schema: 'marketplace'"));
        });
