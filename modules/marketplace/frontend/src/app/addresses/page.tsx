...
import {
  MapPin, Plus, Edit2, Trash2, CheckCircle, Star, Phone, ArrowLeft,
  X, Home, Briefcase, Building2,
} from 'lucide-react';
...
function AddressField({
  id, label, value, onChange, error, required, placeholder, inputMode, maxLength, mono,
}: {
  id: string;
  label: string;
  value: string;
  ...
  return (
    <div>
      ...
      />
      {error && (
        <p id={`${id}-error`} role="alert" className="mt-1 text-xs font-medium text-red-600">{error}</p>
      )}
    </div>
  );
}
...
export default function AddressBookPage() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', line1: '', line2: '', city: '', state: '', pincode: '', type: 'home' as 'home' | 'work' | 'other' });
  ...
  const setDefault = async (id: string) => {
    ...
    try {
      ...
      setActionError(describeFailure(err, 'We could not set that as your default address. Please try again.'));
    }
  };
  ...
  const saveAddress = async () => {
    ...
    try {
      ...
      if (form.pincode.length >= 5) {
        try { logPincodeSearch({ pincode: form.pincode, source: 'address_form', serviceable: true, city: form.city, state: form.state, regionCode: country.code, module: 'marketplace' }); } catch {}
      }
      setShowAdd(false);
      setFieldErrors({});
      await loadAddresses();
    } catch (err) {
      setActionError(describeFailure(err, 'We could not save this address. Please check the details and try again.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const openEdit = (addr: Address) => {
    setForm({ name: addr.name, phone: addr.phone, line1: addr.line1, line2: addr.line2, city: addr.city, state: addr.state, pincode: addr.pincode, type: addr.type });
    setEditId(addr.id); setShowAdd(true);
  };
  ...
  return (
    <div className="max-w-[800px] mx-auto px-3 xs:px-4 py-6 space-y-5 pb-mobile-nav">
      ...
      <div className="space-y-3">
        {loading && [0, 1].map(i => <div key={`sk-${i}`} className="bg-white border border-slate-200 rounded-xl p-5 h-32 animate-pulse" />)}
        {!loading && addresses.length === 0 && (
          <div className="text-center py-16 bg-white border border-slate-200 rounded-xl">
            <MapPin className="w-12 h-12 text-slate-200 mx-auto mb-3" />
            <p className="text-slate-500 font-bold">No saved addresses</p>
            <p className="text-xs text-slate-400 mt-1">Add a delivery address to speed up checkout</p>
          </div>
        )}
        {!loading && addresses.map(addr => {
          const cfg = TYPE_CONFIG[addr.type];
          return (
            <div key={addr.id} className={`bg-white border rounded-xl p-5 relative ${addr.isDefault ? 'border-blue-400 ring-2 ring-blue-100' : 'border-slate-200'}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {addr.isDefault && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200 flex items-center gap-0.5"><Star className="w-3 h-3 fill-emerald-500" />Default</span>}
                  </div>
                  <p className="text-sm text-slate-600">{addr.line1}</p>
                  <p className="text-sm text-slate-600">{addr.line2}</p>
                  <p className="text-sm text-slate-600">{addr.city}, {addr.state} – {addr.pincode}</p>
                  <p className="text-xs text-slate-400 mt-2 flex items-center gap-1"><Phone className="w-3 h-3" />{addr.phone}</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => openEdit(addr)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-blue-600"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => removeAddress(addr.id)} className="p-2 hover:bg-red-50 rounded-lg text-slate-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
              {!addr.isDefault && (
                <button onClick={() => setDefault(addr.id)} className="mt-3 text-xs text-blue-600 font-bold hover:underline">Set as Default</button>
              )}
            </div>
          );
        })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto mx-4">
            <div className="flex items-center justify-between p-5 border-b border-slate-200">
              <h3 className="text-lg font-black text-slate-900">{editId ? 'Edit Address' : 'Add New Address'}</h3>
              <button onClick={() => setShowAdd(false)} className="p-1.5 hover:bg-slate-100 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-5 space-y-4">
              {/* Type selector */}
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Address Type</label>
                <div className="flex gap-2">
                  {Object.entries(TYPE_CONFIG).map(([key, cfg]) => (
                    <button key={key} onClick={() => setForm(f => ({ ...f, type: key as any }))} className={`flex-1 py-2 rounded-lg text-xs font-bold border flex items-center justify-center gap-1.5 ${form.type === key ? cfg.color + ' ring-2 ring-blue-200' : 'border-slate-200 text-slate-500'}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <AddressField id="full-name" label="Full Name" required value={form.name} error={fieldErrors.fullName}
                  onChange={(v) => setForm(f => ({ ...f, name: v }))} />
                <AddressField id="phone" label="Phone" required inputMode="tel" value={form.phone} error={fieldErrors.phone}
                  onChange={(v) => setForm(f => ({ ...f, phone: v }))} />
              </div>
              <AddressField id="address-line-1" label="Address Line 1" required value={form.line1} error={fieldErrors.line1}
                placeholder="House/Flat No., Building Name" onChange={(v) => setForm(f => ({ ...f, line1: v }))} />
              <AddressField id="address-line-2" label="Address Line 2" value={form.line2} error={fieldErrors.line2}
                placeholder="Street, Area, Landmark" onChange={(v) => setForm(f => ({ ...f, line2: v }))} />
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <AddressField id="city" label="City" required value={form.city} error={fieldErrors.city}
                  onChange={(v) => setForm(f => ({ ...f, city: v }))} />
                <AddressField id="state" label="State" required value={form.state} error={fieldErrors.state}
                  onChange={(v) => setForm(f => ({ ...f, state: v }))} />
                <AddressField id="pincode" label={country.address.hasPostalCode ? 'Postal Code' : 'Postal Code (optional)'}
                  required={country.address.hasPostalCode} inputMode="numeric" mono
                  value={form.pincode} error={fieldErrors.postalCode}
                  onChange={(v) => setForm(f => ({ ...f, pincode: v }))} />
              </div>
            </div>
            {actionError && (
              <div role="alert" className="mx-5 mb-1 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-700">
                {actionError}
              </div>
            )}
            <div className="flex gap-3 p-5 border-t border-slate-200">
              ...
              <button onClick={saveAddress} disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-bold py-2.5 rounded-xl text-sm flex items-center justify-center gap-2"><CheckCircle className="w-4 h-4" />{saving ? 'Saving…' : editId ? 'Update' : 'Save Address'}</button>
            </div>
          </div>
        </div>
      )}
