...
import { Trash2, Plus, Minus, Tag, ShieldCheck, Truck, ChevronRight, ShoppingBag, Gift, CreditCard, X, Check, Copy, Sparkles, ChevronDown, ChevronUp, Percent } from 'lucide-react';
...
export default function CartPage() {
  ...
  const updateQty = (id: string, delta: number) => {
    ...
    if (!current) return;
    cart.update(id, current.qty + delta);
  };

  const removeItem = (id: string) => cart.remove(id);
   ...
   */
  const giftCardCoverage = giftCardApplied ? Math.min(giftCardBalance, subtotal - couponDiscount) : 0;
  ...
  const deliverySaving = delivery === 0 ? MARKETPLACE_DELIVERY_FEE : 0;
  ...
  const handleApplyCoupon = useCallback(async () => {
    ...
    if (!code) return;
    if (couponApplied) { setCouponError('A coupon is already applied'); return; }
  ...
  const handleApplyGiftCard = useCallback(async () => {
    ...
    if (!code) return;
    if (!isAuthenticated) { setGiftCardError('Sign in to use a gift card.'); return; }
    ...
    try {
      ...
      if (balance <= 0) { setGiftCardError('This gift card has no remaining balance.'); return; }
  ...
  const handleRemoveGiftCard = () => { setGiftCardApplied(false); setGiftCard(''); setGiftCardBalance(0); };
    ...
    <div className="bg-slate-50 min-h-screen pb-20">
      <div className="max-w-7xl mx-auto px-3 xs:px-4 pt-6">
        ...
        <h1 className="text-2xl font-black text-slate-900 mb-6 flex items-center gap-2">
          ...
          Your Cart
          <span className="text-base font-semibold text-slate-400 ml-1">({items.reduce((s, i) => s + i.qty, 0)} items)</span>
        </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl px-5 py-3 flex items-center gap-3">
                ...
                <p className="text-sm text-blue-800 font-medium">
                  {delivery === 0
                    ...
                    : `Add ${fmt(amountToFreeDelivery(subtotal))} more for FREE delivery`}
                </p>
              </div>
                  ...
                  <div key={item.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        {hasMrp && (
                          <>
                            <span className="text-sm text-slate-400 line-through">{fmt(item.mrp)}</span>
                            <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">{disc}% off</span>
                          </>
                        )}
                        {item.qty > 1 && (
                          <span className="text-xs text-slate-500">× {item.qty} = <span className="font-bold text-slate-700">{fmt(item.price * item.qty)}</span></span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-4">
                        ...
                        <div className="flex items-center gap-1 border border-slate-200 rounded-lg overflow-hidden">
                          ...
                          <span className="w-9 text-center font-bold text-slate-900 text-sm">{item.qty}</span>
                          <button
                ...
                {couponApplied ? (
                  <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-green-700">SAVE5 applied — 5% off!</span>
                    </div>
                    <button onClick={handleRemoveCoupon} className="text-red-500 hover:text-red-700 p-1" aria-label="Close"><X className="w-4 h-4" /></button>
                  </div>
                ) : (<>
                    ...
                    <button onClick={() => setShowAvailableCoupons(!showAvailableCoupons)} className="text-sm text-blue-600 font-bold mt-3 flex items-center gap-1 hover:text-blue-700">
                ...
                {showAvailableCoupons && !couponApplied && (
                  <div className="mt-3 space-y-2 border-t border-slate-100 pt-3">
                    {availableCoupons.length === 0 && (
                      <p className="text-xs text-slate-500 py-2">No coupons are available right now.</p>
                    )}
                    {availableCoupons.map(c => (
                      <div key={c.code} className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-3 group hover:bg-blue-50 transition-colors">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-mono font-bold text-blue-700 text-sm bg-blue-50 px-2 py-0.5 rounded border border-dashed border-blue-300">{c.code}</span>
                            {c.minOrderValue > subtotal && <span className="text-[10px] text-amber-600 font-bold">Min: {fmt(c.minOrderValue)}</span>}
                          </div>
                          {c.description && <p className="text-xs text-slate-500 mt-1">{c.description}</p>}
                        </div>
                        <button onClick={() => handleCopyCoupon(c.code)} className="flex items-center gap-1 text-xs font-bold text-blue-600 bg-blue-100 hover:bg-blue-200 px-3 py-1.5 rounded-lg transition-colors">
                  ...
                  <div className="flex items-center justify-between bg-purple-50 border border-purple-200 rounded-xl px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-purple-500 mt-1">
                        Balance: {fmt(giftCardBalance)} · Covers up to {fmt(giftCardCoverage)} of this order
                      </p>
                    ...
                    <button onClick={handleRemoveGiftCard} className="text-red-500 hover:text-red-700 p-1" aria-label="Close"><X className="w-4 h-4" /></button>
                  </div>
                ) : (<>
                    <div className="flex gap-3">
                      <input
                        value={giftCard}
                        onChange={(e) => { setGiftCard(e.target.value.toUpperCase()); setGiftCardError(''); }}
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                ...
                <div className="space-y-2">
                  {bankOffers.length === 0 && (
                    <p className="text-xs text-slate-500">No card offers are running at the moment.</p>
                  )}
                  {bankOffers.map((offer) => (
                    <div key={offer.id} className="flex items-start gap-3 bg-green-50/50 border border-green-100 rounded-xl px-4 py-3">
                      <Sparkles className="w-4 h-4 text-green-600 mt-0.5 shrink-0" />
                      <div className="flex-1">
                        <p className="text-xs text-slate-700">{offer.text}</p>
                        {offer.code && <span className="text-[10px] font-mono font-bold text-green-700 bg-green-100 px-1.5 py-0.5 rounded mt-1 inline-block">Use: {offer.code}</span>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 lg:sticky lg:top-[calc(var(--mp-header-h)+1rem)]">
                <h2 className="font-bold text-slate-900 text-lg mb-5 pb-3 border-b border-slate-100">Order Summary</h2>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between text-slate-600">
                    <span>Delivery</span>
                    <span className={delivery === 0 ? 'text-green-600 font-semibold' : 'font-semibold text-slate-900'}>
