 ...
 *
 * The provider stack is not duplicated here: it comes from <AppShell> in
 * shared-ui, which the shell's root layout also uses, so both surfaces mount an
 * identical tree and a provider added later reaches both.
 */
const outfit = Outfit({ subsets: ['latin'], variable: '--font-outfit', display: 'swap' });
const inter = Inter({ subsets: ['latin'], variable: '--font-inter', display: 'swap' });

// Carried over from the former app/marketplace/layout.tsx. Every route in this
// zone is marketplace, so that file had no distinct level to occupy once the
// routes moved: its metadata and its client wrapper belong to the zone root.
export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  ...moduleMeta('marketplace'),
};
...
export default async function MarketplaceZoneLayout({ children }: { children: React.ReactNode }) {
  const cookieStore = await cookies();
  const headerStore = await headers();

  // Same resolution the shell performs, so a customer crossing from / into
  // /marketplace keeps their market and language rather than snapping back to
  // the default on the first zone request.
    ...
    >
      ...
      <body className="bg-slate-50 min-h-screen text-slate-900 antialiased">
        {/*
          ...
          <main id="main-content"> that the marketplace layout renders; it is
          not rendered here because that layout already owns one and nesting
          <main> elements is invalid.
        */}
        <a
          href="#main-content"
        ...
        <AppShell
          ...
          country={country}
        >
          <MarketplaceClientLayout>{children}</MarketplaceClientLayout>
        </AppShell>
      </body>
    </html>
  );
