...
function SearchPageContent() {
  ...
  const matches = (p: any, skip?: 'brand' | 'category') => {
    ...
    const matchBrand =
      skip === 'brand' || selectedBrands.length === 0 || selectedBrands.includes(p.brand);
    const matchCat =
      skip === 'category' || selectedCats.length === 0 || selectedCats.includes(p.category);
  ...
  const brandFacets = facetCounts(
    allProducts.filter((p) => matches(p, 'brand')),
    'brand',
  );
  const categoryFacets = facetCounts(
    allProducts.filter((p) => matches(p, 'category')),
    'category',
  );
  ...
  if (sort === 'price_asc') results = [...results].sort((a, b) => a.price - b.price);
    ...
    results = [...results].sort(
      (a, b) => discountPct(b.mrp, b.price) - discountPct(a.mrp, a.price),
    );
      ...
      <div className="bg-white border-b border-slate-200 sticky top-(--mp-header-h) z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <form onSubmit={handleSearch} className="flex gap-2 items-center">
            ...
            <button
              type="submit"
              className="bg-blue-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-blue-700 transition-colors"
            >
              Search
            </button>
          </form>
          {q && (
            <p className="text-xs text-slate-500 mt-2">
              Showing <span className="font-bold text-slate-900">{results.length}</span> results for{' '}
            ...
            <div className="mb-5">
              ...
              <div className="space-y-2">
                {[4, 3, 2].map((r) => (
                  ...
                  >
                    <Star
                      className={`w-4 h-4 ${minRating === r ? 'fill-blue-500 text-blue-500' : 'fill-yellow-400 text-yellow-400'}`}
                    />
                    {r}★ & above
                  </button>
                ))}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-sm text-slate-700"
                  placeholder="Min"
                />
                <input
                  type="number"
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden md:flex items-center gap-1 border border-slate-200 rounded-lg overflow-hidden">
                <button
                  ...
                  className={`p-1.5 ${viewMode === 'grid' ? 'bg-blue-50 text-blue-600' : 'text-slate-400'}`}
                >
                ...
                <button
                  ...
                  className={`p-1.5 ${viewMode === 'list' ? 'bg-blue-50 text-blue-600' : 'text-slate-400'}`}
                >
          ...
          {activeFilters > 0 && (
            <div className="flex flex-wrap gap-2 mb-3">
              {selectedBrands.map((b) => (
                <span
                  key={b}
                  className="flex items-center gap-1 bg-blue-50 text-blue-700 text-xs font-bold px-3 py-1 rounded-full border border-blue-200"
                >
                  {b}{' '}
                  <button title={`Remove ${b} filter`} onClick={() => toggleBrand(b)}>
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
              {selectedCats.map((c) => (
                <span
                  key={c}
                  className="flex items-center gap-1 bg-emerald-50 text-emerald-700 text-xs font-bold px-3 py-1 rounded-full border border-emerald-200"
                >
                  {c}{' '}
                  <button title={`Remove ${c} filter`} onClick={() => toggleCat(c)}>
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
              {minRating > 0 && (
                <span className="flex items-center gap-1 bg-amber-50 text-amber-700 text-xs font-bold px-3 py-1 rounded-full border border-amber-200">
                  {minRating}★+{' '}
                  <button title="Remove rating filter" onClick={() => setMinRating(0)}>
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <button
                ...
                className="bg-blue-600 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-blue-700"
              >
                Retry
              </button>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 z-10 p-1.5 bg-white rounded-full shadow-sm border border-slate-100"
                    >
                      <Heart
                        className={`w-4 h-4 ${wishlist.has(p.id) ? 'fill-red-500 text-red-500' : 'text-slate-300'}`}
                      />
                    </button>
                    <Link href={zoneHref(productPath(p))}>
                      ...
                      <div className="p-3">
                        ...
                        <h3 className="text-sm font-medium text-slate-800 line-clamp-2 leading-snug mb-1.5">
                          {p.title}
                        </h3>
            ...
            <div className="space-y-3">
              {results.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow flex"
                  >
                    <Link href={zoneHref(productPath(p))} className="flex flex-1">
                      ...
                      <div className="p-4 flex-1">
                        ...
                        <div className="flex items-baseline gap-2">
                          ...
                          {disc > 0 && (
                            <span className="text-sm text-green-600 font-bold">{disc}% off</span>
                          )}
                        </div>
                        <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                          <Truck className="w-3 h-3 text-slate-400" /> {p.delivery}
                        </p>
                      </div>
                    </Link>
                    <div className="flex flex-col justify-center gap-2 p-4 shrink-0">
                      ...
                      <button
                        onClick={() => toggleWishlist(p.id)}
                        className={`text-xs font-bold py-2 px-4 rounded-lg border transition-colors whitespace-nowrap ${wishlist.has(p.id) ? 'border-red-300 text-red-500 bg-red-50' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                      >
        ...
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 bg-white z-[201] lg:hidden rounded-t-2xl shadow-2xl max-h-[80vh] overflow-y-auto animate-in slide-in-from-bottom-4">
            <div className="flex items-center justify-between px-4 py-4 border-b border-slate-100">
              ...
              <div className="flex items-center gap-3">
                {activeFilters > 0 && (
                  <button onClick={clearAll} className="text-xs text-red-600 font-semibold">
                    Clear All
                  </button>
                )}
                <button
