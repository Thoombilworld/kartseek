...
/**
 * KARTSEEK — drop the dead second market column on `pharmacy.pharmacy_stores`.
 *
 * This table carried TWO market columns. `region_code` is the one every scope
 * check in this module reads; `countryCode` defaulted to `'KEN'` and held `'IND'` on all six rows while
 * `region_code` held the real `'IN'`. Two market columns, one
...
export class DropDeadMarketColumns1786502400000 implements MigrationInterface {
  ...
  public async up(q: QueryRunner): Promise<void> {
    const table = 'pharmacy.pharmacy_stores';
    if (!(await this.hasTable(q, table))) return;
    if (!(await this.hasColumn(q, 'pharmacy', 'pharmacy_stores', 'countryCode'))) {
      console.warn(
