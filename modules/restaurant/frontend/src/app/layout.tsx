...
/**
 * Root layout for the restaurant zone.
 *
...
// Carried over from the former app/restaurant/layout.tsx. Every route in this
// zone is restaurant, so that file had no distinct level left to occupy once the
// routes moved: its metadata moved here and its chrome became <RestaurantShell>.
export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  ...moduleMeta('restaurant'),
};
...
export default async function RestaurantZoneLayout({ children }: { children: React.ReactNode }) {
  const cookieStore = await cookies();
  const headerStore = await headers();

  // Same resolution the shell performs, so a customer crossing from / into
  // /restaurant keeps their market and language rather than snapping back to the
    ...
    >
      ...
      <body className="bg-slate-50 min-h-screen text-slate-900 antialiased">
        {/*
          ...
          <main id="main-content"> that the restaurant chrome renders; it is not
