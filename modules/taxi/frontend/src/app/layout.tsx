...
// Carried over from the former app/taxi/layout.tsx. Every route in this
// zone is taxi, so that file had no distinct level left to occupy once the
...
export default async function TaxiZoneLayout({ children }: { children: React.ReactNode }) {
  const cookieStore = await cookies();
  const headerStore = await headers();

  // Same resolution the shell performs, so a customer crossing from / into
  // /taxi keeps their market and language rather than snapping back to the
    ...
    >
      ...
      <body className="bg-slate-50 min-h-screen text-slate-900 antialiased">
        {/*
          ...
          <main id="main-content"> that the taxi layout renders; it is not
        ...
        */}
        ...
        <AppShell
          ...
          country={country}
        >
          <TaxiClientLayout>{children}</TaxiClientLayout>
        </AppShell>
      </body>
    </html>
  );
