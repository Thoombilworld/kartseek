...
export function renderReadmeBlock(s) {
  ...
  if (NEST_KINDS.includes(s.kind)) {
    lines.push(
      `Health: ${s.health.live ? `${code(s.health.live)} (live)` : 'no HTTP health route yet'}${s.health.ready ? `, ${code(s.health.ready)} (ready)` : ''}.`,
    );
    lines.push(
      s.database
        ? `Database: ${code(s.database.name)}, schema ${code(s.database.schema)} (${code(`${s.database.envPrefix}_*`)}).`
        : 'Database: none (owns no tables).',
    );
    lines.push(
      `Image: ${code(s.image)}. Depends on: ${s.dependsOn.join(', ') || 'nothing'}.${s.kafka ? ` Kafka group: ${code(s.kafka.groupId)}.` : ''}`,
    );
  } else {
    lines.push(
      s.kind === 'web-zone'
        ? `Mounted by the shell at ${code(s.basePath)}; open it through http://localhost:3000${s.basePath}.`
...
function renderRows(reg) {
  return reg.services.map((s) => {
    const db = NEST_KINDS.includes(s.kind)
      ? s.database
        ? `${s.database.name} / ${s.database.schema}`
        : '—'
      : '—';
...
export function generateAll(reg, root, { check = false } = {}) {
  ...
  const targets = [
    ...
    ...reg.services.map((s) => ({
      rel: `${s.path}/README.md`,
      next: (cur) => replaceBlock(cur, renderReadmeBlock(s)),
      whole: false,
    })),
  ];
  for (const t of targets) {
    ...
    if (!t.whole && current === null) {
      result.skipped.push(`${t.rel} (no README yet)`);
      continue;
    }
    const next = t.whole ? t.next() : t.next(current);
    if (next === null) {
      result.skipped.push(`${t.rel} (no ${START} markers)`);
      continue;
    }
