...
test('a zone must have a basePath and a shell must not', () => {
  assert.ok(
    validateShape({ version: 1, services: [{ ...zone, basePath: undefined }] }).some((p) =>
      p.includes('basePath'),
    ),
  );
  assert.ok(
    validateShape({ version: 1, services: [{ ...zone, kind: 'web-shell', name: 'web' }] }).some(
      (p) => p.includes('basePath'),
    ),
  );
});
